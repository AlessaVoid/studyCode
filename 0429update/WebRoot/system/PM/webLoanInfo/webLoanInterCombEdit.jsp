...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#takenType").bind("change", function () {
            ...
            if (takenType === '1') {
                ...
                var initCombUrl = "<%=path%>/webLoan/selectLoanCombByParentId.htm?combParentCode=000000";
                testData.rows = [];
                $.ajax({
                        type: 'GET',
                        url: initCombUrl,
    ...
    var combTakenGrid;
    var testData = {
        "form.paginate.pageNo": 1, "form.paginate.totalRows": 13, "rows": []
    };
    ...
    function initComplete() {
        var combUrl = "<%=path%>/webLoan/listAllLoanCombByLevelV2.htm?combLevel=0";
        $.ajax({
                type: 'GET',
                url: combUrl,
                async: false,
            ...
            columns: [
                    ...
                    editor: {
                        ...
                        render: function (item) {
                            for (var i = 0; i < combList["list"].length; i++) {
                                if (combList["list"][i]['value'] === item.combId)
                ...
                },
                {
                    ...
                    editor: {
                        ...
                        render: function (item) {
                            for (var i = 0; i < combList["list"].length; i++) {
                                if (combList["list"][i]['value'] === item.combTakenId)
                                    return combList["list"][i]['key']
                            }
                            return item.combTakenId;
                        }
                    }
                }
            ],
    ...
    function sub() {
        ...
        top.Dialog.confirm("是否保存信息?", function () {
            ...
            $.post(partUrl, {
                "gridData": gridData,
                "parentCombCode": '000000',
                "takenType": takenType
            }, function (result) {
                if (result.success === "true" || result.success === true) {
                    top.Dialog.alert("配置组间占用规则成功");
                } else {
    ...
    function deleteUser() {
        ...
        for (var index in rows) {
            combTakenGrid.deleteRow(rows[index]);
        }
    }
