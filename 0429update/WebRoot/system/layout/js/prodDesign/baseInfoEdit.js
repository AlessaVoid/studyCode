...
//根据产品代码和产品品牌控制产品品牌是否可以选择(有复制新增)
function brandIsDisabledCopy(){
	var brandCodeValue = $("#brandCodeValue").val();
	var prodCode = $("#prodCode").val();
	var prodType = $("#prodType").val();
	if(prodCode != null && prodCode != "" && brandCodeValue != "" && brandCodeValue != null && prodType !="copy"){
		$("#brandCode").attr("disabled","disabled");
		$("#brandCode").render();
	}
}
...
function validateProdtCharactType(value){
	$.post(path + "/design/validateProdtCharactType.htm",{"prodCode":$("#prodCode").val(),"prodtCharactType":value},function(result){
		if(result.success == "true" || result.success == true){
