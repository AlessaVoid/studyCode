...
function initComplete() {
	...
	grid = $("#dataBasic").quiGrid({
				columns : [ {
							...
							url : '<%=path%>/taHkNotifyInfoG/findPage.htm',
							sortName : '',
							rownumbers : true,
							checkbox : true,
							height : '100%',
...
function onInfo() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/taHkNotifyInfoG/infoUI.htm?tradeDate="+rows[0].tradeDate+"&tradeKindCode=" + rows[0].tradeKindCode+"&accountingFlag=" + rows[0].accountingFlag+"&prodCode=" + rows[0].prodCode,
				"详细信息",800,600);
	}
}
//审批
function onApply() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/taHkNotifyInfoG/applyUI.htm?tradeDate="+rows[0].tradeDate+"&tradeKindCode=" + rows[0].tradeKindCode+"&accountingFlag=" + rows[0].accountingFlag+"&prodCode=" + rows[0].prodCode,
				"审批信息",800,600);
	}
	}
}
...
function onExport(){
	top.Dialog.confirm('是否导出全部数据?',	function() {
			$("#queryForm").attr("action","<%=path%>/taHkNotifyInfoG/exportExcelFile.htm");
			$("#queryForm").submit();
		});									
}
//打印
function onPrint(){
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		showDialog("<%=path%>/taHkNotifyInfoG/printUI.htm?tradeDate="+rows[0].tradeDate+"&tradeKindCode=" + rows[0].tradeKindCode+"&accountingFlag=" + rows[0].accountingFlag+"&prodCode=" + rows[0].prodCode,
				"详细信息",800,600);
	}								
}
</script>
