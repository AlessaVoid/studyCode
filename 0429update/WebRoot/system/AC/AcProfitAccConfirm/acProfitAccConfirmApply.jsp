	...
	<div class="basicTabModern">
		...
		<table class="tableStyle" useMultColor="true" mode="list">
			...
			<tr>
				...
				<td colspan="3">
				 <c:if test="${entity.tradeKindCode==151}">
			...
			<tr>
				<td align="right">总金额（小）：</td>
				<c:if test="${entity.tradeKindCode==151}">	
					 <c:choose>
						  <c:when test="${entity.isProfitFeeToRMB!=1}">
						     <td colspan="3">${entity.sumAmt}</td>
						  </c:when>
						   <c:when test="${entity.isProfitFeeToRMB==1}">
						     <td colspan="3">---</td>
						  </c:when>
			...
			<tr>
				...
				<c:if test="${entity.tradeKindCode==151}">	
					<c:choose>
						<c:when test="${entity.isProfitFeeToRMB!=1}">
						       <td colspan="3"><fmt:formatMoneyUp fmtValue="${entity.sumAmt},${entity.currency}" fmtClass="width:100%;" 
								id="moneyUP" /></td>
						</c:when>
						<c:when test="${entity.isProfitFeeToRMB==1}">
						       <td colspan="3">---</td>
		...
		<div name="复核信息" style="width:100%;height:80px;">
			...
			<table class="tableStyle" width="97%" mode="list" formMode="line">
				...
				<tr>
					<td colspan="4">
						<div align="center">
							<button type="button" onclick="return doSubmit('form1','<%=path%>/acProfitAccConfirm/doApply.htm')" class="saveButton"/>
