			...
			.quiGrid(
					{
						...
						url : '<%=path%>/acCostAccConfirm/findPage.htm',
						sortName : 'tradeDate',
						rownumbers : true,
						checkbox : true,
						height : '100%',
	...
	if(selectOneRow(grid)){
		...
		if(costAccFlag=='0'){
			top.Dialog.confirm('本金未到账，是否继续?',function() {
				showDialog("<%=path%>/acCostAccConfirm/apply.htm?&tradeDate=" + rows[0].tradeDate+"&prodCode="+rows[0].prodCode+"&custType=0&tradeKindCode="+rows[0].tradeKindCode,"到账确认申请信息",500,400);
				});
		}else{
			showDialog("<%=path%>/acCostAccConfirm/apply.htm?&tradeDate=" + rows[0].tradeDate+"&prodCode="+rows[0].prodCode+"&custType=0&tradeKindCode="+rows[0].tradeKindCode,"到账确认申请信息",500,400);
			}
