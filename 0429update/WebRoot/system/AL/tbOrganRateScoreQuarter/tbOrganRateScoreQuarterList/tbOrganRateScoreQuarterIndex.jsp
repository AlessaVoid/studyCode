...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/tbOrganRateScoreQuarter/tbOrganRateScoreMonthData.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onUpdate() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/tbOrganRateScoreQuarter/updateTbOrganRateScoreMonthPage.htm?id=" + rows[0].id, "修改评分", 1800, 600);
        }
    }
    ...
    function onSubmit() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbOrganRateScoreQuarterApp/commiTbOrganRateScoreMonthUI.htm?id=" + rows[0].id, "提交评分", 940, 580);
        }
    }
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbOrganRateScoreQuarter/tbOrganRateScoreMonthDetailPage.htm?id=" + rows[0].id, "评分详情信息", 1800, 600);
        }
    }
    ...
    function onInsert() {
        ...
        top.Dialog.confirm("确定要生成评分吗？", function () {
            ...
            $.post("<%=path%>/tbOrganRateScoreQuarter/addTbOrganRateScoreMonth.htm", {
            }, function (result) {
                //消除加载loading
                div.unmask();
