...
<script>
 ...
 function transFormat(amount) {
    ...
    function initAuditOper() {
        $.ajax({
            url: "<%=path%>/tbOrganRateScoreMonthPendingApp/getOperInfoListByRolecode.htm?taskid=${taskid}",
            method: "GET",
            async: false,
    ...
    function onAudit(isAgree) {
        ...
        top.Dialog.confirm("确定要提交该记录吗？", function () {
            ...
            $.ajax({
                url: "<%=path%>/tbOrganRateScoreMonthPendingApp/auditLoanRateScoreRequest.htm",
                data: {
                    "id": id,
                    "taskId": taskId,
                    "comment": comment,
