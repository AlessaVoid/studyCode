...
<script>
 ...
 function transFormat(amount) {
    ...
    function submitInfo() {
        ...
        top.Dialog.confirm("确定要提交审批吗？", function () {
            ...
            $.ajax({
                url: '<%=path%>/tbOrganRateScoreMonthApp/startTbOrganRateScoreMonthAudit.htm',
                method: 'GET',
                data: {
                    "auditOper": auditOper,
                    "comment": comment,
 ...
 if(menu_id==undefined){
    ...
    function initAuditOper() {
        //初始化评分审批人员
        $.ajax({
            url: "<%=path%>/tbOrganRateScoreMonthApp/getOperInfoListByRolecode.htm?rolecode=${rolecode}",
            method: "GET",
            async: false,
