...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/punishApplyPendingApp/getPendingAppTbLineOver.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onAudit() {
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/punishApplyPendingApp/tbQuotaApplyAuditUI.htm?id=" + rows[0].id + "&taskid=" + rows[0].taskid , "审批罚息申请", 1780, 680);
        } else {
            top.Dialog.alert("请选择一条罚息申请");
        }
    }
    //查看流程图
    function onViewImg() {
