...
<head>
    <meta charset="UTF-8" />
    <title>信贷到期量日报表</title>
    <%@include file="/common/common_edit.jsp" %>
    <link rel="stylesheet" href="${path}/libs/css/report/defaultTheme.css" />
    <link href="${path}/libs/css/report/report.css" rel="stylesheet" media="screen" />
...
<body>
<div class="box2_custom" boxType="box2" panelTitle="信贷到期量日报表" id="searchPanel">
    <form action="${path}/report/reportCreditMaturityDaily.htm?menuNo=RE-02" id="queryForm" method="post" onsubmit="οnsubmitForm();">
...
<div>
    <div class="divider">
        <table class="fancyTable" id="myTable" cellpadding="0" cellspacing="0" style="table-layout:fixed;word-wrap:break-word;">
            <thead>
            <tr>
                <th style="width:200px" class="first"><span class="title1">贷种组合</span><br><span class="title2">贷款到期量余额(万元)</span><br><span class="title3">机构</span></th>
            ...
            <c:forEach items="${organList}" var="organ">
                <tr>
                    ...
                    <c:forEach items="${combList}" var="comb">
                        ...
                        <c:set var="dayLimit" value="${baseInfoMap[baseInfo_key]}" />

                        <c:choose>
                            <c:when test="${!empty dayLimit}">
                                <td align="right"><fm:formatNumber maxFractionDigits="4" value="${dayLimit*0.0001}"></fm:formatNumber></td>
                            </c:when>
                            <c:otherwise>
                                <td align="right">0</td>
                            </c:otherwise>
...
<script>
    ...
    function download() {
        ...
        location = "<%=path%>/report/reportCreditMaturityDailyDownload.htm?statisticsDate=" + statisticsDate ;
    }
