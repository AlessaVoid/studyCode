				...
				if (valid) {
					$.post("<%=path%>/design/checkCustPropInfo.htm", $("#form1").serialize(), function(result) {
						if (result.success == "true" || result.success == true) {
							$.post("<%=path%>/designRollback/updateCustPropInfo.htm", $("#form1").serialize(), function(result) {
	...
	<body>
		...
		<table class="tableStyle tab-hei-30" width="80%" mode="list" formMode="line">
				<tr>
					<td>与或关系类型：</td>
					<td>
						<dic:select dicType="Y_HREFER"  dicNo="${entity.relType }"  name="relType"  tgClass="validate[required]"></dic:select>
