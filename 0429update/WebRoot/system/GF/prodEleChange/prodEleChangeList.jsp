			...
			.quiGrid(
					{
						...
						url : '<%=path%>/gfProdEleChange/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function showDialogSave(url,title,width,height){
	if(width == ''){
		width = 600;
	}
	if(height == ''){
...
function onInfo() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/gfProdEleChange/infoUI.htm?prodCode=" + rows[0].prodCode,
				"详细信息",1280,680);
	}
}
//修改
function onUpdate() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		$.post("<%=path%>/gfProdEleChange/validateModify.htm",{prodCode:rows[0].prodCode},function(result){
					if (result.success=='success') 
							top.Dialog.open({
							URL:"<%=path%>/gfProdEleChange/editUI.htm?prodCode="+rows[0].prodCode,
							Title:"修改信息",
							Width:1280,
							Height:680,
							ShowCloseButton:false
...
function exportFile(){
	top.Dialog.confirm('是否导出全部数据?',	function() {
			$("#queryForm").attr("action","<%=path%>/gfProdEleChange/exportProdparaFile.htm");
			$("#queryForm").submit();
		});									
}
function search(){
