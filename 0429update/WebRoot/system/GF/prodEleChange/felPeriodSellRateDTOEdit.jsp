...
<script type="text/javascript">
	...
	felPeriodProfitGrid = $("#felPeriodProfitParam").quiGrid({
           ...
           { display: '期限下限(含,天)', name: 'periodLow', align: 'center', width: "15%",
        	   editor:{type:"text",maxlength:10,inputMode:"numberOnly"}
           },
           { display: '期限上限(天)', name: 'periodHigh', align: 'center', width: "15%",
        	   editor:{type:"text",maxlength:10,inputMode:"numberOnly"}
           },
           { display: '客户收益率(%)', name: 'profitRate', align: 'center', width: "10%",
           ...
           { display: '启用状态', name: 'status', align: 'center', width: "15%",
        	   render : function(rowdata) {
        		   if (rowdata.status == "1") {
        				return "正常";
	...
	function appendFelPeriodProfit(){
	    ...
	    diag.Title = "选择机构";
	    diag.URL = "<%=path%>/system/GF/prodEleChange/chooseOgran.jsp";
	    diag.OKEvent = function(){
	        var inputValue = diag.innerFrame.contentWindow.getOrganCodes();
	        if (inputValue=='') {
	        	top.Dialog.alert("必须选择至少一个机构");
	        	return;
			}
	        var systemDate="${systemDate}";
	        systemDate=calDay(systemDate,1);
	       var organCodes= inputValue.split(",");
    	  for (var int = 0; int < organCodes.length; int++) {
	        	var organCode=organCodes[int];
	    		var felPeriodProfit = {
	    			organCode :organCode,
	    			periodLow: "0",
	    			periodHigh: "7",
	...
	function showImportDialog(){
		showDialog("<%=path%>/gfProdEleChange/uploadUI.htm","数据导入",350,130);
	}
	//解析参数模板
	function import_(fileName) {
		Dialog.close();
		$.post("<%=path%>/gfProdEleChange/readXsl.htm?fileName=" + fileName +"&prodCode="+$("#prodCode").val(),function(result) {
			var a = eval(result);
			showInfo(a);
		}, "json");
	}
	function showInfo(list){
		...
		for(var i=0;i<list.length;i++){
			felPeriodProfit ={
				organCode : list[i].organCode,
				periodLow: list[i].periodLow,
				periodHigh: list[i].periodHigh,
				profitRate: list[i].profitRate,
				sellRate : list[i].sellRate,
				beginDate: list[i].beginDate,
				custType : list[i].custType,
...
function search(){
	var organCode=$("#selectTree1").attr("relValue");
	$.post("<%=path%>/gfProdEleChange/findFelPeriodSellRateDTOInfo.htm",{
		organCode:organCode,
		prodCode:"${prodCode}"
	},function(result) {
		felPeriodProfitGrid.loadData(result);
	}, "json");
}
</script>
	</head>
	<body onclick="fmtText()">
		...
		<div align="center">
			<button type="button" onclick="return sub('form1','<%=path%>/gfProdEleChange/compareFelPeriodSellRateDTOInfo.htm')" class="saveButton"/>
		</div>
		</form>
	</body>
</html>