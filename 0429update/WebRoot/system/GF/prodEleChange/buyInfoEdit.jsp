...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function sub(formId,url) {
			$(".money-2").each(function(){rmoney(this);});
			var valid = $("#"+formId).validationEngine({
				returnIsValid : true
			});
			if (valid) {
				var planIssueAmt = $("#planIssueAmt").val();
				if(planIssueAmt=="0.00" || planIssueAmt==undefined || planIssueAmt==""){
	...
	<form id="form1">
		...
		<table class="tableStyle tab-hei-30" width="80%" mode="list" formMode="line">
			...
			<tr>
				<td>募集结束日期（到）：</td>
				<td>
					<input type="text" id="raisingEndDate" name="raisingEndDate" class="date validate[required]" dateFmt="yyyyMMdd" maxlength="8" value="${entity.raisingEndDate }"/>
					<span class="star">*</span>
				</td>
				<td>募集结束时间（到）：</td>
				<td>
			...
			<tr>
				<td>计划募集金额（${webProdBaseInfo.currency }）：</td>
				<td>
					<input name="planIssueAmt" id="planIssueAmt" value="${entity.planIssueAmt }" class="validate[required,length[0,18]] money-2" type="text"/>
					<span class="star">*</span>
				</td>
			...
			<tr>
				<td colspan="4">
					<div align="center">
						<button type="button" onclick="return sub('form1','<%=path%>/gfProdEleChange/compareBuyInfo.htm')" class="saveButton"/>
					</div>
				</td>
			</tr>
		</table>
