...
<html>
	...
	<body>
		...
		<div class="box2_custom" boxType="box2" panelTitle="查询条件" id="searchPanel">
			<form id="queryForm" method="post" action="<%=path%>/webResearchRollback/findPage.htm">
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/webResearchRollback/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onEdit() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/webResearchRollback/reSubmitUI.htm?prodCode="+ rows[0].prodCode+"&taskId="+rows[0].taskId,"产品预研重编辑",1280,680);
	}
}
...
function onInfo() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		showDialog("<%=path%>/webResearchRollback/infoUI.htm?prodCode=" + rows[0].prodCode+"&taskId=" + rows[0].taskId,"产品详细信息",1280,680);
	}
}
...
function onExportExcel(){
	top.Dialog.confirm('是否导出全部数据?',	function() {
			$("#queryForm").attr("action","<%=path%>/webResearchRollback/exportProdparaFile.htm");
			$("#queryForm").submit();
	});									
}
</script>
