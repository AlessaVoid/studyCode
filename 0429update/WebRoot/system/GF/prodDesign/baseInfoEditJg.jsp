			...
			$(".money").each(function(){rmoney(this);});
					...
					if(result.success=='true'||result.success==true){
						top.Dialog.confirm(result.msg, function() {
							$.post(url+"?brandCodeTemp="+brandCodeTemp, $("#"+formId).serialize(), function(result) {
								if (result.success == "true" || result.success == true) {
...
<form id="form1">
	...
	<table class="tableStyle tab-hei-30" width="80%" mode="list" formMode="line">
		...
		<tr>
			...
			<c:choose>
					<c:when test="${empty entity.vipGrade}">
						<dic:select tgClass="validate[required]" name="vipGrade" dicType="VIP_GRADE" dicNo="0"></dic:select>
					</c:when>
					<c:otherwise>
				<dic:select tgClass="validate[required]" name="vipGrade" dicType="VIP_GRADE" dicNo="${entity.vipGrade}"></dic:select>
					</c:otherwise>
				</c:choose>
				<span class="star">*</span>
			</td>
		...
		<tr>
			...
			<td>
				<dic:select name="isDividendAlter" dicType="IS_YES" tgClass="validate[required]" dicNo="${entity.isDividendAlter}"></dic:select>
				<span class="star">*</span>
			</td>
			<td align="right">
				是否允许财产证明：
