...
<html>
	...
	<body>
		...
		<div class="box2_custom" boxType="box2" panelTitle="查询条件" id="searchPanel">
			<form action="<%=path%>/design/findPage.htm" id="queryForm" method="post">
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/design/findDesignPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onDesign() {
	if(selectOneRow(grid)){
		...
		top.Dialog.open({
			URL : "<%=path%>/design/prodDesignAddUI.htm?prodType=1&prodCode="+rows[0].prodCode,
			Title : "个人产品设计",
			ID : 'designAdd',
			Width : 1280,
			Height : 680
...
function onImport(){
	top.Dialog.open({
		URL : "<%=path%>/system/GF/prodDesign/ImportDialog.jsp",
		Title : "机构产品导入",
		Width : 600,
		Height : 480
	});
...
//机构专户产品新增
function onAdd() {
	showDialog("<%=path%>/design/prodAddUI.htm?prodType=2","新增信息",1280,680);
}

//机构专户复制新增
function onCopy(){
	top.Dialog.open({
		URL : "<%=path%>/system/GF/prodDesign/prodDesignCopy.jsp",
		Title : "复制新增",
		Width : 500,
		Height :260
	});
...
function onExport() {
	top.Dialog.confirm('是否导出全部数据?',	function() {
		$("#queryForm").attr("action","<%=path%>/design/exportProdparaFile.htm");
		$("#queryForm").submit();
	});	
}
...
function onCancel() {
	if(selectOneRow(grid)){
		...
		top.Dialog.confirm("确定要作废该产品吗?", function() {
			$.post("<%=path%>/design/cancelProduct.htm", {"prodCode":rows[0].prodCode}, function(result) {
...
//解析参数模板
function importParamsInfo(filePath){
	top.Dialog.open( {
		URL : "<%=path%>/design/readLhqx.htm?filePath="+filePath,
		Title : "模板导入",
		Width : 900,
		Height : 1000
	});
