...
<html xmlns="http://www.w3.org/1999/xhtml">
			...
			top.Dialog.confirm("是否保存信息?", function() {
				$.post(url, $("#"+formId).serialize(), function(result) {
					if (result.success == "true" || result.success == true) {
						top.Dialog.alert("保存成功！",function(t){
	...
	<form id="form1">
		<input type="hidden" id="prodCode" value="${prodCode}" name="prodCode"/>
			...
			<input type="hidden" id="prodOperModel" value="${prodOperModel}" name="prodOperModel"/>
		...
		<table class="tableStyle tab-hei-30" width="80%" mode="list" formMode="line">
			...
			<tr>
				<td colspan="4">
					<div align="center">
						<button type="button" onclick="return sub('form1','<%=path%>/webResearchAppInfo/saveBuyInfo.htm')" class="saveButton"/>
					</div>
				</td>
			</tr>
		</table>
