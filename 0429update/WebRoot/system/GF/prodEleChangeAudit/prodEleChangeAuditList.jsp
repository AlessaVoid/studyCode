			...
			.quiGrid(
					{
						...
						url : '<%=path%>/eleChangeAudit/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onAudit() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/eleChangeAudit/auditUI.htm?taskId=" + rows[0].taskId, "产品要素变更审批",1280,680);
	}
}
//待签收任务
function onRevTask() {
	showDialog("<%=path%>/eleChangeAudit/revTaskUI.htm","待签收任务",1280,680);
}
...
function onInfo() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		showDialog("<%=path%>/eleChangeAudit/infoUI.htm?prodCode="+rows[0].prodCode+"&processInstanceId="+rows[0].processInstanceId,
				"详细信息",1280,680);
	}
}
...
function onExportExcel() {
	top.Dialog.confirm('是否导出全部数据?',	function() {
		$("#queryForm").attr("action","<%=path%>/eleChangeAudit/exportFile.htm");
		$("#queryForm").submit();
	});		
}
