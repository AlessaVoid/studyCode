...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/tbTradeManger/single/findPage.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onSubmit() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/tbTradeManger/singleCommit/commitTbQuotaUI.htm?qaId=" + rows[0].qaId,
                "提交审批", 1000, 480);
    ...
    function onInsert() {
        var rows = grid.getSelectedRows();
        showDialog("<%=path%>/tbTradeManger/single/insertUI.htm", "录入信息", 1060, 480);
        grid.loadData();
    }
    ...
    function onUpdate() {
        if (selectOneRow(grid)) {
            ...
            var rows = grid.getSelectedRows();
            if (rows[0].qaState <= 2) {
                showDialog("<%=path%>/tbTradeManger/single/updateUI.htm?qaId=" + rows[0].qaId,
                    "修改信息", 1000, 480);
            } else if (rows[0].qaState >= 4) {
                top.Dialog.alert("已提交,无法在做修改！", null, null, null, 5);
                return;
            }
        }
        grid.loadData();
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbTradeManger/single/infoUI.htm?qaId=" + rows[0].qaId,
                "详细信息", 1000, 480);
        }
    }
