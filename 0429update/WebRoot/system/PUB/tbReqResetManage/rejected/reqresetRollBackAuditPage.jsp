...
<script type="text/javascript">
        ...
        grid = $("#dataBasic").quiGrid(
            {
                ...
                url: '<%=path%>/TbReqresetReject/listAllRollBackTaskList.htm',
                sortName: '',
                rownumbers: true,
                checkbox: true,
                height: '100%',
    ...
    function onResubmit() {
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/TbReqresetReject/loanReqResubmitAuditUI.htm?reqresetId=" + rows[0].reqresetid + "&taskid=" + rows[0].taskid, "重新编辑", 1280, 680);
        } else {
    ...
    function onRollBackDetail() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/TbReqresetReject/listReqRejectDetailAuditUI.htm?reqresetId=" + rows[0].reqresetid + "&processInstanceId=" + rows[0].procinstid, "信贷需求详细信息", 1280, 680);
        }
    }
