...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/tbTradeManger/lineOver/findPage.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onSubmit() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/tbTradeManger/lineOverCommit/commitTbQuotaUI.htm?qaId=" + rows[0].qaId,
                "提交审批", 1000, 480);
    ...
    function onInsert() {
        var rows = grid.getSelectedRows();
        showDialog("<%=path%>/tbTradeManger/lineOver/insertUI.htm", "录入信息", 1060, 480);
        grid.loadData();
    }
    ...
    function onUpdate() {
        if (selectOneRow(grid)) {
            ...
            var rows = grid.getSelectedRows();
            if (rows[0].qaState <= 2) {
                showDialog("<%=path%>/tbTradeManger/lineOver/updateUI.htm?qaId=" + rows[0].qaId,
                    "修改信息", 1000, 480);
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbTradeManger/lineOver/infoUI.htm?qaId=" + rows[0].qaId,
                "详细信息", 1000, 480);
        }
    }
