...
<script type="text/javascript">
        ...
        grid = $("#dataBasic").quiGrid(
            {
                ...
                url: '<%=path%>/tbPlanadjStripe/loadAllLoanadjPlanInfo.htm',
                rownumbers: true,
                checkbox: true,
                height: '100%',
                width: "100%",
    ...
    function enterReport() {
        showDialog("<%=path%>/tbPlanadjStripe/toEnterReport.htm", "导入批量条线调整计划", 1000, 600);
    }
    ...
    function showCommitLoanPlanAdjustInfoPage() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/tbPlanadjStripeApp/commitTbPlanadjUI.htm?planadjId=" + rows[0].planadjid, "提交批量条线调整计划", 940, 580);
        }
    }
    ...
    function showPlanadjOhterMonth() {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanadjStripe/loadLoanPlanadjDetailInfoInsertPage.htm", "新增批量条线调整计划", 940, 580);
    }
    ...
    function showLoanPlanAdjustUpdatePage() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanadjStripe/listTbPlanadjDetailAuditUI.htm?planadjId=" + rows[0].planadjid,
                "修改批量条线调整计划详情", 940, 580);
        }
    }
    ...
    function deleteLoanPlanAdjustInfo() {
        if (selectOneRow(grid)) {
            ...
            if (rows[0].planStatus >= 8) {
                top.Dialog.alert("已提交审批的批量条线调整计划不允许删除");
                return;
            }
            top.Dialog.confirm("确定要删除该记录吗？", function () {
                //这里换成后端删除计划调整的url
                var planAdjustDeleteUrl = "<%=path%>/tbPlanadjStripe/deleteTbPlanadjDetail.htm";
    ...
    function viewLoanPlanAdjustDetailInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanadjStripe/TbPlanadjInfo.htm?planadjId=" + rows[0].planadjid,
                "批量条线调整计划详情信息", 940, 580);
        }
    }
