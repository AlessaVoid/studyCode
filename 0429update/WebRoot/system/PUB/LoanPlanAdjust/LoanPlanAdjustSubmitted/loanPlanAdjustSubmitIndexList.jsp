...
<script type="text/javascript">
        ...
        grid = $("#dataBasic").quiGrid(
            {
                ...
                url: '<%=path%>/tbPlanadjSub/getSubmitAuditHistoryRecord.htm',
                sortName: '',
                rownumbers: true,
                checkbox: true,
                height: '100%',
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/tbPlanadjSub/listTbPlanSubmitDetailAuditUI.htm?planadjId=" + rows[0].planadjid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1500, 700);
        }
    }
    ...
    function onDownload() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            location = "<%=path%>/tbPlanadjSub/downloadPlan.htm?planadjId=" + rows[0].planadjid;
        }
    }
