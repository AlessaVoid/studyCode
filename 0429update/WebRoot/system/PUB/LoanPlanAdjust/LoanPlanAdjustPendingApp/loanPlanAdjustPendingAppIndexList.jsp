...
<script type="text/javascript">
        ...
        grid = $("#dataBasic").quiGrid(
            {
                ...
                url: '<%=path%>/tbPlanadjPendingApp/getPendingAppTbPlanadj.htm',
                sortName: '',
                rownumbers: true,
                checkbox: true,
                height: '100%',
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/tbPlanadjPendingApp/listTbPlanPendingDetailAuditUI.htm?planadjId=" + rows[0].planadjid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1500, 700);
        }
    }
    ...
    function onAudit() {
        var rows = grid.getSelectedRows();
        if (rows.length === 1) {
            showDialog("<%=path%>/tbPlanadjPendingApp/listTbPlanDetailAuditUI.htm?planadjId=" + rows[0].planadjid + "&taskId=" + rows[0].taskid, "审批批量机构调整计划", 1600, 800);
        } else {
