...
<script type="text/javascript">
    ...
    function initComplete() {
        ...
        $.post("<%=path%>/tbTradeManger/tbQuotaApply/findComb.htm",
            {"organLevel":level}, function (result) {
                //赋给data
    ...
    function initAuditOper() {
        //这里是个关键点
        $.ajax({
            url: "<%=path%>/overApplyReject/getOperInfoListByRolecode.htm?taskid=${taskid}&qaId=${TbOver.qaId}",
            method: "GET",
            async: false,
            success: function (result) {
    ...
    function submitInfo() {
        ...
        $.ajax({
            url: '<%=path%>/overApplyReject/update.htm',
            type: 'POST',
            data: fd,
            processData: false,
            contentType: false,
        ...
        top.Dialog.confirm("确定要提交该记录吗？", function () {
            ...
            if (!valid) {
                ...
                fd.append('lastUserType', ${lastUserType});
                    $.ajax({
                        url: "<%=path%>/overApplyReject/auditLoanQuotaApplyRequest.htm",
                        data: fd,
                        method: 'POST',
                        processData: false,
                        contentType: false,
        ...
        <tr>
            ...
            <td colspan="3">
                <textarea id="qaReason" name="qaReason" AUTOCOMPLETE="off" style="width:90%;"
                          class="validate[required]">${TbOver.qaReason}</textarea>
            </td>
        </tr>
    </table>
</form>
