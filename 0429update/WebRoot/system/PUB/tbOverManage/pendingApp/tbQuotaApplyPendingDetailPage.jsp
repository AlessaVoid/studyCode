...
<script type="text/javascript">
    ...
    function initAuditOper() {
        //这里是个关键点
        $.ajax({
            url: "<%=path%>/overApplyPendingApp/getOperInfoListByRolecode.htm?taskid=${taskid}&qaId=${TbOver.qaId}",
            method: "GET",
            async: false,
            success: function (result) {
    ...
    function onAudit(isAgree) {
        ...
        if(0==isAgree){
            top.Dialog.confirm("确定要驳回至原录入员吗？", function () {
                ...
                $.ajax({
                    url: "<%=path%>/overApplyPendingApp/auditLoanQuotaApplyRequest.htm",
                    data: {
                        "qaId": qaId,
                    ...
                    success: function(res) {
                        if (res.success === "true" || res.success === true) {
                            top.Dialog.alert("申请审批驳回成功!",function(){
