...
<script type="text/javascript">
    ...
    function initAuditOper() {
        //这里是个关键点
        $.ajax({
            url: "<%=path%>/TbQuotaApplyPendingApp/getOperInfoListByRolecode.htm?taskid=${taskid}&qaId=${TbQuotaApply.qaId}",
            method: "GET",
            async: false,
    ...
    function onAudit(isAgree) {
        ...
        if(0==isAgree){
            ...
            top.Dialog.confirm("确定要驳回至原录入员吗？", function () {
                ...
                $.ajax({
                    url: "<%=path%>/TbQuotaApplyPendingApp/auditLoanQuotaApplyRequest.htm",
                    data: {
                        "qaId": qaId,
                        "taskId": taskId,
                        "comment": comment,
