...
<script type="text/javascript">
    ...
    function initAuditOper() {
        //这里是个关键点
        $.ajax({
            url: "<%=path%>/tbTradeManger/lineTbLineTempCommit/getOperInfoListByRolecode.htm?rolecode=${rolecode}"+"&qaId="+qaId,
            method: "GET",
            async: false,
    ...
    function submitInfo() {
        ...
        top.Dialog.confirm("确定要提交审批吗？", function () {
            ...
            $.ajax({
                url: '<%=path%>/tbTradeManger/lineTbLineTempCommit/startLoanQuotaApplyAudit.htm',
                method: 'GET',
                data: {
                    "auditOper": auditOper,
                    "qaId": qaIdStr,
                    ...
                    if (res.success === 'true' || res.success === true) {
                        top.Dialog.alert("单一条线临时额度申请提交成功,等待审批结果", function () {
 ...
 if(menu_id==undefined){
                        ...
                        top.Dialog.alert("单一条线临时额度申请提交失败");
                        $("#btn1").removeAttr("disabled");
                        $("#btn2").removeAttr("disabled");
                    }
                }
