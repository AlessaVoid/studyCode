...
<script type="text/javascript">
    ...
    function initAuditOper() {
        //这里是个关键点
        $.ajax({
            url: "<%=path%>/lineTbLineTempReject/getOperInfoListByRolecode.htm?taskid=${taskid}&qaId=${TbLineTemp.qaId}",
            method: "GET",
            async: false,
    ...
    function submitInfo() {
        ...
        $.ajax({
            url: '<%=path%>/lineTbLineTempReject/update.htm',
            type: 'POST',
            data: fd,
            processData: false,
            contentType: false,
        ...
        top.Dialog.confirm("确定要提交该记录吗？", function () {
            ...
            if (!valid) {
                ...
                return;
                    ...
                    $.ajax({
                        url: "<%=path%>/lineTbLineTempReject/auditLoanQuotaApplyRequest.htm",
                        data: fd,
                        method: 'POST',
                        processData: false,
                        contentType: false,
        ...
        <tr>
            ...
            <td>
                <input type="text" id="qaAmt" name="qaAmt" onblur='upperCase(this)' AUTOCOMPLETE="off"
                       class=" validate[required,custom[onlyNumberWideSpecial]] " value="${TbLineTemp.qaAmt}"
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" id="qaStartDate" name="qaStartDate" class="validate[required] date"
                       value="${TbLineTemp.qaStartDate}" AUTOCOMPLETE="off"
                       dateFmt="yyyyMMdd"
            ...
            <td>
                ...
                <input type="text" id="qaExpDate" name="qaExpDate" class="validate[required] date"
                       value="${TbLineTemp.qaExpDate}" AUTOCOMPLETE="off"
                       dateFmt="yyyyMMdd"
                       onfocus="WdatePicker({skin:themeColor,minDate:'%y-%M-%d',maxDate:maxDateStr})"/>
                <span class="star">*</span>
            </td>
