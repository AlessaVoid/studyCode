...
<script type="text/javascript">
        ...
        grid = $("#dataBasic").quiGrid(
            {
                ...
                url: '<%=path%>/TbLineReqReject/listAllRollBackTaskList.htm',
                sortName: '',
                rownumbers: true,
                checkbox: true,
                height: '100%',
    ...
    function onResubmit(){
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/TbLineReqReject/loanReqResubmitAuditUI.htm?lineReqId=" + rows[0].linereqid + "&taskid=" + rows[0].taskid , "重新编辑", 1280, 680);
        } else {
    ...
    function onRollBackDetail() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/TbLineReqReject/listReqRejectDetailAuditUI.htm?lineReqId=" + rows[0].linereqid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1280, 680);
        }
    }
