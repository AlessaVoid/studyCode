...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/tbOrganRateScoreMonthReject/listAllRollBackTaskList.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onRollBackDetail() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbOrganRateScoreMonthReject/listRateScoreRejectDetailAuditUI.htm?id=" + rows[0].id + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1280, 680);
        }
    }
    ...
    function onResubmit() {
        var rows = grid.getSelectedRows();
        if (rows.length === 1) {
            showDialog("<%=path%>/tbOrganRateScoreMonthReject/rateScoreResubmitAuditUI.htm?id=" + rows[0].id + "&taskid=" + rows[0].taskid, "重新编辑", 1280, 680);
        } else {
