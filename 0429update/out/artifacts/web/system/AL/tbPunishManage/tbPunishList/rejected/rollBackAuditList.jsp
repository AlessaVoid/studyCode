...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/punishApplyReject/listAllRollBackTaskList.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onResubmit() {
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/punishApplyReject/tbQuotaApplyResubmitAuditUI.htm?id=" + rows[0].id + "&taskid=" + rows[0].taskid, "重新编辑", 1780, 680);
        } else {
            top.Dialog.alert("请选择一条临时额度申请");
        }
    }
    ...
    function onRollBackDetail() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/punishApplyReject/rejectDetailAuditUI.htm?id=" + rows[0].id + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1780, 680);
        }
    }
