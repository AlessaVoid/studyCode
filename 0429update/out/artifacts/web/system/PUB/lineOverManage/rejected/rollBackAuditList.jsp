...
<script type="text/javascript">
    ...
    function initComplete() {
        ...
        $.post("<%=path%>/tbTradeManger/lineOver/showComb.htm",
            {}, function (result) {
                organMap = result.combMap;
                grid.loadData();
            }, "json");

        $.post("<%=path%>/tbTradeManger/tbReqList/showOrgan.htm",
            {}, function (result) {
                newOrganMap = result.organMap;
                grid.loadData();
            }, "json");
        //当提交表单刷新本页面时关闭弹窗
        top.Dialog.close();
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/lineOverApplyReject/listAllRollBackTaskList.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onResubmit(){
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/lineOverApplyReject/tbQuotaApplyResubmitAuditUI.htm?qaid=" + rows[0].qaid + "&taskid=" + rows[0].taskid , "重新编辑", 1280, 680);
        } else {
            top.Dialog.alert("请选择一条申请记录");
        }
    }
    ...
    function onRollBackDetail() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/lineOverApplyReject/rejectDetailAuditUI.htm?qaId=" + rows[0].qaid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1280, 680);
        }
    }
