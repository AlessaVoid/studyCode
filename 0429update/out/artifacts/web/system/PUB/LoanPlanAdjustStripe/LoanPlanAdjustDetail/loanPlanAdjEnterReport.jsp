...
<head>
    ...
    <script type="text/javascript">
        ...
        function xhrSubmit() {
            ...
            top.Dialog.confirm("确定要录入"+planMonth+"的批量条线调整计划吗?|操作提示", function () {
                $('#submit').attr('disabled', true);
                $.ajax({
                    url: '<%=path%>/tbPlanadjStripe/enterReportPlanadjByMonth.htm',
                    type: 'POST',
                    data: fd,
 ...
 if(menu_id==undefined){
                        ...
                        if (planadjStatus == 0) {
                            top.Dialog.alert(" 所属月份批量条线调整计划已经填写，可以导入");
                        } else if (planadjStatus == 32) {
                            top.Dialog.alert(" 所属月份批量条线调整计划已被驳回，可以导入");
                        } else if (planadjStatus == 8) {
                            top.Dialog.alert(" 所属月份批量条线调整计划正在审批中，不能导入");
                            return;
                        }
...
<div class="form-group">
    ...
    <a href="<%=path%>/tbPlanadjStripe/downPlanadjTemplate.htm?type=2">
