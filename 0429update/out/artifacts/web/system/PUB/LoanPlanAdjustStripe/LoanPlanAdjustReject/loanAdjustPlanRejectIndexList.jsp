...
<script type="text/javascript">
        ...
        grid = $("#dataBasic").quiGrid(
            {
                ...
                url: '<%=path%>/tbPlanadjStripeReject/listAllRollBackTaskList.htm',
                sortName: '',
                rownumbers: true,
                checkbox: true,
                height: '100%',
    ...
    function onResubmit() {
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/tbPlanadjStripeReject/loanTbPlanadjResubmitAuditUI.htm?planadjId=" + rows[0].planadjid + "&taskId=" + rows[0].taskid, "重新编辑", 1280, 680);
        } else {
    ...
    function onRollBackDetail() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanadjStripeReject/listTbPlanadjRejectDetailAuditUI.htm?planadjId=" + rows[0].planadjid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1280, 680);
        }
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanSub/listTbPlanSubmitDetailAuditUI.htm?planId=" + rows[0].planid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1280, 680);
        }
    }
