    ...
    <form action="" id="queryForm" method="post">
        <table class="tableStyle" mode="list" formMode="line" style="width: 100%;">
            <tr>
                <td>所属月份：</td>
                <td width="20">
                    <input name="qaMonth" type="text" id="sys_time" class="input-small inline form_datetime"
                           style="width: 160px;"/>
...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/tbTradeManger/tbQuotaApply/findPage.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onSubmit() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/tbTradeManger/tbQuotaApplyCommit/commitTbQuotaUI.htm?qaId=" + rows[0].qaId,
                "提交审批", 1000, 580);
    ...
    function onInsert() {
        ...
        //删除记录
        $.post("<%=path%>/tbTradeManger/tbQuotaApply/checkPlan.htm", {},
            function (result) {
                if (result.success == "true" || result.success == true) {
                    showDialog("<%=path%>/tbTradeManger/tbQuotaApply/insertUI.htm", "录入信息", 1060, 480);
                    grid.loadData();
                } else {
    ...
    function onUpdate() {
        if (selectOneRow(grid)) {
            ...
            var rows = grid.getSelectedRows();
            if (rows[0].qaState <= 2) {
                showDialog("<%=path%>/tbTradeManger/tbQuotaApply/updateUI.htm?qaId=" + rows[0].qaId,
                    "修改信息", 1000, 480);
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbTradeManger/tbQuotaApply/infoUI.htm?qaId=" + rows[0].qaId,
                "详细信息", 1000, 480);
        }
    }
