...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    columns: [

                        {
                            ...
                            render: function (rowdata, rowindex, value, column) {
                                ...
                                return "<font color=black style='font-weight:900;'>"+dateStr+"</font>";
                            }
                        }, {
                            display: '当前状态',
                            name: 'reqresetState',
                    ...
                    url: '<%=path%>/tbTradeManger/tbReqresetList/showFindPage.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onSubmit() {
        if (selectOneRow(grid)) {
            ...
            if (rows[0].reqresetState >= 4) {
                top.Dialog.alert("该需求已提交,本次操作无效！", null, null, null, 5);
                return;
            }
            ...
            top.Dialog.confirm("确定要提交该需求吗？", function () {
                //删除记录
                $.post("<%=path%>/tbTradeManger/tbReqresetDetail/submit.htm", {
    ...
    function onInsert() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            if (rows[0].reqresetState == 1) {
                showDialog("<%=path%>/tbTradeManger/tbReqresetDetail/insertUI.htm?reqresetId=" + rows[0].reqresetId, "录入信息", 960, 780);
            } else {
    ...
    function onUpdate() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            if (rows[0].reqresetState == 2) {
                showDialog("<%=path%>/tbTradeManger/tbReqresetDetail/updateUI.htm?reqresetId=" + rows[0].reqresetId,
                    "修改信息", 860, 680);
            } else if (rows[0].reqresetState >= 4) {
                top.Dialog.alert("该需求已提交,无法再做修改！", null, null, null, 5);
                return;
            } else if (rows[0].reqresetState == 1) {
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            if (rows[0].reqresetState > 1) {
            showDialog("<%=path%>/tbTradeManger/tbReqresetDetail/infoUI.htm?reqresetId=" + rows[0].reqresetId,
                "详细信息", 860, 680);
            } else {
                top.Dialog.alert("该需求未录入,暂无法查看详情！", null, null, null, 5);
                return;
            }
        }
    }
    ...
    function onSubmit() {
        if (selectOneRow(grid)) {
            ...
            if (rows[0].reqresetState >= 4) {
                ...
                //提交记录
                showDialog("<%=path%>/tbReqResetSubmit/commitTbReqresetUI.htm?reqresetId=" + rows[0].reqresetId,
                    "提交审批", 860, 480);
