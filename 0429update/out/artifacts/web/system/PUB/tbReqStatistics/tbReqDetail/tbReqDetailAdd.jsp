...
<script type="text/javascript">
    ...
    function sub(testNum) {
        ...
        $.ajax({
            type: "POST",
            url: "<%=path%>/tbTradeManger/tbReqDetail/insert.htm",
            data: {"reqUnit": reqUnit, "tbReqDetail": tbReqDetail, "reqId": reqId, "state": 2, "testNum": testNum},
            dataType: "json",
            success: function (result) {
                if (result.success == true || result.success == "true") {
                    ...
                    if (result.code == '403') {
                        ...
                        top.Dialog.alert("当前批次的需求已录入，请转至菜单查看", function () {
                            var menu_id = parent.getCurrentTabId();
                            if (menu_id == undefined) {
                                top.Dialog.close();
                                return;
