...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/overApplyReject/listAllRollBackTaskList.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onResubmit(){
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/overApplyReject/tbQuotaApplyResubmitAuditUI.htm?qaid=" + rows[0].qaid + "&taskid=" + rows[0].taskid , "重新编辑", 1280, 680);
        } else {
            top.Dialog.alert("请选择一条单一机构计划调整申请");
        }
    }
    ...
    function onRollBackDetail() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/overApplyReject/rejectDetailAuditUI.htm?qaId=" + rows[0].qaid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1280, 680);
        }
    }
