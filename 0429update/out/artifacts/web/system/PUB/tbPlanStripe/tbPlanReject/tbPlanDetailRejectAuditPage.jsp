...
<form id="form1" >
    ...
    <div align="center" style="alignment: center;margin:20px;">
        <button type="button" id="updateTbPlan" onclick="return sub()" class="saveButton"></button>
        <button type="button" id="cancelUpdateTbPlan" onclick="return cancel()" class="cancelButton"></button>
    </div>
</form>
...
<script>
 ...
 function transFormat(amount) {
    ...
    function initAuditOper() {
        $.ajax({
                url: "<%=path%>/tbPlanStripeReject/getOperInfoListByRolecode.htm?taskid=${taskid}",
                method: "GET",
                async: false,
    ...
    function submit(isAgree) {
        ...
        $.ajax({
            url: "<%=path%>/tbPlanStripeReject/auditLoanTbPlanRequest.htm",
            data: {
                "planId": planId,
                "taskId": taskId,
                "comment": comment,
