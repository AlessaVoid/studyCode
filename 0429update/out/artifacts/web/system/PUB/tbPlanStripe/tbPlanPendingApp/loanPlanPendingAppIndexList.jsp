...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/tbPlanStripePendingApp/getPendingAppTbPlan.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onAudit() {
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/tbPlanStripePendingApp/listTbPlanDetailAuditUI.htm?planId=" + rows[0].planid + "&taskid=" + rows[0].taskid, "审批计划", 1280, 680);
        } else {
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanStripePendingApp/listTbPlanPendingDetailAuditUI.htm?planId=" + rows[0].planid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1280, 680);
        }
    }
