...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/creditPlanStripe/creditPlanData.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    //新增
    function onInsert() {
        showDialog("<%=path%>/creditPlanStripe/toAddCreditPlan.htm", "新增条线计划", 1200, 680);
    }

    //导入计划
    function enterReport() {
            showDialog("<%=path%>/creditPlanStripe/toEnterReportPlan.htm", "导入条线计划", 1200, 680);
    }
    ...
    function onUpdate() {
        if (selectOneRow(grid)) {
            ...
            if (rows[0].planStatus >= 8) {
                top.Dialog.alert("已提交审批的条线计划不允许更新");
                return;
            }
            showDialog("<%=path%>/creditPlanStripe/toUpdateCreditPlan.htm?planId=" + rows[0].planid, "修改条线计划", 1200, 680);
        }
    }
    ...
    function onDelete() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            if (rows[0].planStatus >= 8) {
                top.Dialog.alert("已提交审批的条线计划不允许删除");
                return;
            }
            if (rows[0].planStatus === -1) {
                top.Dialog.alert("条线计划已经删除,重复操作");
                return;
            }
            top.Dialog.confirm("确定要删除该条线计划记录吗？", function () {
                $.post("<%=path%>/creditPlanStripe/deleteCreditPlan.htm", {
                    planId: rows[0].planid
                }, function (result) {
                    if (result.success === "true" || result.success === true) {
                        top.Dialog.alert("删除条线计划成功");
                        grid.loadData();
                    } else {
    ...
    function onSubmit() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanStripe/commitTbPlanUI.htm?planId=" + rows[0].planid, "提交条线计划", 1200, 680);
        }
    }
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/creditPlanStripe/toDetailCreditPlan.htm?planId=" + rows[0].planid, "计划详情信息", 1200, 680);
        }
    }
