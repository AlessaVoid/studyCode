...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/tbPlanPendingApp/getPendingAppTbPlan.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onAudit() {
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/tbPlanPendingApp/listTbPlanDetailAuditUI.htm?planId=" + rows[0].planid + "&taskid=" + rows[0].taskid, "审批机构计划", 1000, 680);
        } else {
            top.Dialog.alert("请选择一条机构计划");
        }
    }
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanPendingApp/listTbPlanPendingDetailAuditUI.htm?planId=" + rows[0].planid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1000, 680);
        }
    }
