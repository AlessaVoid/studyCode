...
<script type="text/javascript">
            ...
            .quiGrid(
                {
                    ...
                    url: '<%=path%>/tbPlanSub/getSubmitAuditHistoryRecord.htm',
                    sortName: '',
                    rownumbers: true,
                    checkbox: true,
                    height: '100%',
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/tbPlanSub/listTbPlanSubmitDetailAuditUI.htm?planId=" + rows[0].planid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1000, 680);
        }
    }
    ...
    function onDownload() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            location = "<%=path%>/tbPlanSub/downloadPlan.htm?planId=" + rows[0].planid;
        }
    }
