...
<script type="text/javascript">
        ...
        grid = $("#dataBasic").quiGrid(
            {
                ...
                url: '<%=path%>/TbLoanReqSub/getSubmitAuditHistoryRecord.htm',
                sortName: '',
                rownumbers: true,
                checkbox: true,
                height: '100%',
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/TbLoanReqSub/listReqSubmitDetailAuditUI.htm?reqId=" + rows[0].reqid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1280, 680);
        }
    }
    ...
    function onViewImg() {
        if (selectOneRow(grid)) {
            ...
            showDialog("<%=path%>/workflow/imageUI.htm?taskId=" + rows[0].taskId + "&processInstanceId="+rows[0].procinstid,"当前流程图",1480,780);
        }
    }
