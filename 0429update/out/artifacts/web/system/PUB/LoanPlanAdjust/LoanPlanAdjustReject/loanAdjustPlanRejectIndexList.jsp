...
<script type="text/javascript">
        ...
        grid = $("#dataBasic").quiGrid(
            {
                ...
                url: '<%=path%>/tbPlanadjReject/listAllRollBackTaskList.htm',
                sortName: '',
                rownumbers: true,
                checkbox: true,
                height: '100%',
    ...
    function onResubmit() {
        ...
        if (rows.length === 1) {
            showDialog("<%=path%>/tbPlanadjReject/loanTbPlanadjResubmitAuditUI.htm?planadjId=" + rows[0].planadjid + "&taskId=" + rows[0].taskid, "重新编辑",1500, 700);
        } else {
    ...
    function onRollBackDetail() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanadjReject/listTbPlanadjRejectDetailAuditUI.htm?planadjId=" + rows[0].planadjid + "&processInstanceId=" + rows[0].procinstid, "详细信息",1500, 700);
        }
    ...
    function onInfo() {
        if (selectOneRow(grid)) {
            var rows = grid.getSelectedRows();
            showDialog("<%=path%>/tbPlanSub/listTbPlanSubmitDetailAuditUI.htm?planId=" + rows[0].planid + "&processInstanceId=" + rows[0].procinstid, "详细信息", 1500, 700);
        }
    }
