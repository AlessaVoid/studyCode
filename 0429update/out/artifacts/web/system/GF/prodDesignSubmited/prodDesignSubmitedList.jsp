...
<html>
	...
	<body>
		...
		<div class="box2_custom" boxType="box2" panelTitle="查询条件" id="searchPanel">
			<form action="<%=path%>/designSubmited/findPage.htm" id="queryForm" method="post">
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/designSubmited/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onInfo() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/designSubmited/infoUI.htm?prodCode="+ rows[0].prodCode+"&taskId="+rows[0].taskId+"&processInstanceId="+rows[0].processInstanceId,"产品详细信息",1280,680);
	}
}
...
function onViewAppTable() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		$.post("<%=path%>/designSubmited/judgment.htm?prodCode=" + rows[0].prodCode,function(result){
			if(result.success=='true'||result.success==true){
				showDialog("<%=path%>/designSubmited/prodDesignAppTableInfo.htm?prodCode=" + rows[0].prodCode + "&processInstanceId="+rows[0].processInstanceId,"产品审核表",700,680);
			}else{
...
function onExport() {
	top.Dialog.confirm('是否导出全部数据?',	function() {
		$("#queryForm").attr("action","<%=path%>/designSubmited/exportProdparaFile.htm");
		$("#queryForm").submit();
	});	
}
</script>
