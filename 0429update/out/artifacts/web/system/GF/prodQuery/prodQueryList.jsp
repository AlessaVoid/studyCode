			...
			<form action="" id="queryForm" method="post">
				<table class="tableStyle"  mode="list" formMode="line" style="width: 97%;">
					...
					<tr>
						...
						<td>
							<div class="suggestion" name="prjName" matchName="prjName" 
									url="<%=path%>/prodQuery/selectPrjName.htm" suggestMode="remote"></div>
						</td>
						<td align="right">
							产品状态：
						</td>
						<td>
							<div class="selectTree" url="<%=path%>/prodQuery/getProdStatusDic.htm?dicType=PROD_STATUS" multiMode="true" allSelectable="true" exceptParent="true"  name="prodStatus" ></div>
						</td>
					</tr>
					<tr>
						<td align="right">
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/prodQuery/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onExportExcel(){
	top.Dialog.confirm('是否导出全部数据?',	function() {
		$("#queryForm").attr("action","<%=path%>/prodQuery/exportExcel.htm");
		$("#queryForm").submit();
	});									
}
</script>
