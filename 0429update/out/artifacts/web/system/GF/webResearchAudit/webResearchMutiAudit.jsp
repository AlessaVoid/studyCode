			...
			.quiGrid(
					{
						...
						url : '<%=path%>/webResearchAudit/mutiAppFindPage.htm?taskId=${taskId}',
						sortName : '',
						enabledEdit : true,
						rownumbers : true,
						checkbox : true,
...
function onOk(){
	...
	if(rows.length == 0){
		...
		map.value = "1";
		param.push(map);
		$.post("<%=path%>/webResearchAudit/mutiAuditAppCheck.htm",param,function(result) {
			if (result.success == "true" || result.success == true) {
				var lastUserType = result.msg.split("-")[1];
				if(lastUserType == true){
					$.post("<%=path%>/webResearchAudit/mutiAudit.htm",param,function(result) {
						if (result.success == "true" || result.success == true) {
					...
					var url = "<%=path%>/webResearchAudit/mutiAuditAppUser.htm?isAgree=1"+"&roleCodeList="+result.msg;
					top.Dialog.open({
						URL : url,
						Title : "审批信息",
						Width : "500",
