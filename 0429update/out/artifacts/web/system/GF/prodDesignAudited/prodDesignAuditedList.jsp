	...
	<body>
		...
		<div class="box2_custom" boxType="box2" panelTitle="查询条件" id="searchPanel">
			<form action="<%=path%>/designAudited/findPage.htm" id="queryForm" method="post">
				<input type="hidden"  value="${prodStatus}" id="prodStatus"/>
				<table class="tableStyle"  mode="list" formMode="line" style="width: 97%;">
					...
					<tr>
						...
						<td>
							<div class="selectTree" url="<%=path%>/gfDict/getPordStatusTreeDic.htm?dicType=D_PROD_STATUS" multiMode="true" allSelectable="true" exceptParent="true" name="prodStatus" selectedValue="${prodStatus }" params='{"prodStatus":"${prodStatus}"}'></div>
						</td>
						<td>
							币种：
						</td>
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/designAudited/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onInfo() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/designAudited/infoUI.htm?prodCode="+ rows[0].prodCode+"&taskId="+rows[0].taskId+"&processInstanceId="+rows[0].processInstanceId,"产品详细信息",1280,680);
	}
}
...
function onViewImg() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
	//	showDialog("<%=path%>/workflow/showResource.htm?processDefinitionId=" + rows[0].processDefinitionId + "&resourceType=image","当前流程图",1280,680);
		if(rows[0].taskId == ""){
			showDialog("<%=path%>/workflow/showResource.htm?processDefinitionId=" + rows[0].processDefinitionId + "&resourceType=image","当前流程图",1280,680);
		}else{
			showDialog("<%=path%>/workflow/imageUI.htm?taskId=" + rows[0].taskId + "&processInstanceId="+rows[0].processInstanceId,"当前流程图",1280,680);
		}
	}
}
...
function onExport() {
	top.Dialog.confirm('是否导出全部数据?',	function() {
		$("#queryForm").attr("action","<%=path%>/designAudited/exportProdparaFile.htm");
		$("#queryForm").submit();
	});	
}
//查看产品审核表
function onViewAppTable() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		$.post("<%=path%>/designAudited/judgment.htm?prodCode=" + rows[0].prodCode,function(result){
			if(result.success=='true'||result.success==true){
				showDialog("<%=path%>/designAudited/prodDesignAppTableInfo.htm?prodCode=" + rows[0].prodCode + "&processInstanceId="+rows[0].processInstanceId,"产品审核表",700,680);
			}else{
