...
<html>
	...
	<body>
		...
		<div class="box2_custom" boxType="box2" panelTitle="查询条件" id="searchPanel">
			<form action="<%=path%>/designRollback/findPage.htm" id="queryForm" method="post">
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/designRollback/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onEdit() {
	if(selectOneRow(grid)){
		...
		top.Dialog.open({
			URL : "<%=path%>/designRollback/reSubmitUI.htm?taskId=" + rows[0].taskId + "&prodCode=" + rows[0].prodCode,
			Title : "产品设计重编辑",
			ID : 'designRollBack',
			Width : 1280,
			Height : 680
...
function onInfo() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		showDialog("<%=path%>/designRollback/infoUI.htm?prodCode=" + rows[0].prodCode+"&taskId=" + rows[0].taskId,"产品详细信息",1280,680);
	}
}
...
function onExport() {
	top.Dialog.confirm('是否导出全部数据?',	function() {
		$("#queryForm").attr("action","<%=path%>/designRollback/exportProdparaFile.htm");
		$("#queryForm").submit();
	});	
}
...
function onRestore() {
	top.Dialog.open({
		URL : "<%=path%>/designRollback/restoreUI.htm",
		Title : "产品恢复",
		Width : 1280,
		Height : 600
	});
...
function onCancel() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		top.Dialog.confirm("确定要作废该产品吗?", function() {
			$.post("<%=path%>/designRollback/cancelProduct.htm", {"prodCode":rows[0].prodCode}, function(result) {
...
function onViewAppTable() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		$.post("<%=path%>/designRollback/judgment.htm?prodCode=" + rows[0].prodCode,function(result){
			if(result.success=='true'||result.success==true){
				showDialog("<%=path%>/designRollback/prodDesignAppTableInfo.htm?prodCode=" + rows[0].prodCode + "&processInstanceId="+rows[0].processInstanceId,"产品审核表",700,680);
			}else{
				top.Dialog.alert(result.msg);
			}
		}, "json");
