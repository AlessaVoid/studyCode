...
<html>
	...
	<body>
		...
		<div class="box2_custom" boxType="box2" panelTitle="查询条件" id="searchPanel">
			<form action="<%=path%>/webResearchAudited/findPage.htm" id="queryForm" method="post">
				...
				<table class="tableStyle"  mode="list" formMode="line" style="width: 97%;">
					...
					<tr>
						...
						<td>
						<dic:select dicType="IS_YES" id="isAutoBalance" name="isAutoBalance" selWidth="127"></dic:select>
						</td>
						<td>
							待续接标识：
						</td>
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/webResearchAudited/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onInfo() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/webResearchAudited/infoUI.htm?prodCode=" + rows[0].prodCode+"&taskId=" + rows[0].taskId+"&processInstanceId=" + rows[0].processInstanceId,"产品详细信息",1280,680);
	}
}
...
function onExportExcel(){
	top.Dialog.confirm('是否导出全部数据?',	function() {
			$("#queryForm").attr("action","<%=path%>/webResearchAudited/exportProdparaFile.htm");
			$("#queryForm").submit();
	});									
}
</script>
