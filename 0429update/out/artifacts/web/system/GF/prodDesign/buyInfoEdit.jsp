					...
					if (result.success == "true" || result.success == true) {
						top.Dialog.alert(result.msg, function() {
							if(copyType == "copyType"){
								parent.turnPage("panel11","panel12");
							}else{
	...
	<form id="form1">
		...
		<table class="tableStyle tab-hei-30" width="80%" mode="list" formMode="line">
			...
			<tr>
				...
				<td width="31%">
					<input type="text" id="raisingBeginDate" name="raisingBeginDate" class="date validate[required,length[0,8]]" dateFmt="yyyyMMdd" maxlength="8" value="${entity.raisingBeginDate }"/>
				...
				<c:choose>
						...
						<c:choose>
							<c:when test="${ webProdBaseInfo.isNightMarket=='0'}">
							<input type="text" id="raisingBeginTime" name="raisingBeginTime" class="date" dateFmt="HHmmss" maxlength="6" value="090000"/>
							</c:when>
								<c:otherwise>
								<input type="text" id="raisingBeginTime" name="raisingBeginTime" class="date" dateFmt="HHmmss" maxlength="6" value="200000"/>
								</c:otherwise>
						</c:choose>
						</c:when>
						<c:otherwise>
							<c:choose>
							<c:when test="${webProdBaseInfo.isNightMarket=='1'}">
								<input type="text" id="raisingBeginTime" name="raisingBeginTime" class="date" dateFmt="HHmmss" maxlength="6" value="200000"/>
							</c:when>
								<c:otherwise>
			...
			<tr>
				<td colspan="4">
					<div align="center">
						<button type="button" onclick="return sub('form1','<%=path%>/design/saveBuyInfo.htm')" class="saveButton"/>
					</div>
				</td>
			</tr>
		</table>
