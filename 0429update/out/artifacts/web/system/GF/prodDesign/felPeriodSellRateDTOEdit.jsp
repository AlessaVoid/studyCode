...
<script type="text/javascript">
	...
	function showImportDialog(){
		showDialog("<%=path%>/design/uploadUI.htm","数据导入",350,130);
	}
	//解析参数模板
	function import_(fileName) {
		Dialog.close();
		$.post("<%=path%>/design/readXsl.htm?fileName=" + fileName +"&prodCode="+$("#prodCode").val(),function(result) {
			var a = eval(result);
			showInfo(a);
		}, "json");
	}
...
function sub(formId,url) {
	...
	<body onclick="fmtText()">
		...
		<div align="center">
			<button type="button" onclick="return sub('form1','<%=path%>/design/saveFelPeriodSellRateDTO.htm')" class="saveButton"/>
		</div>
	   	</form>
	</body>
</html>