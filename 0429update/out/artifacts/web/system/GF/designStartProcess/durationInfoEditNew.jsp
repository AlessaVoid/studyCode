	...
	function initComplete(){
		...
		if(1!="${isCurrProdt}"){
			...
			durationGrid = $("#durationParam").quiGrid({
	               ...
	               { display: '份额确认日', name: 'quotAffirmDate', align: 'center', width: "5%",
	            	   editor:{type:"date",dateFmt: "yyyyMMdd",detailEdit:"true"}
	               },
	               { display: '启用状态', name: 'status', align: 'center', width: "5%",type: 'STARTFLAG',
	             	   editor:{type:'select',data:startFlag,selWidth:100,boxWidth:90},
...
function sub(formId) {
	...
	$.post("<%=path%>/design/saveDurationInfoNew.htm?openPeriod="+openPeriod+"&prodCode"+prodCode,param,function(result) {
...
function createNewDuration(){
	...
	if(openPhaseModel == '0'){ 
		if (valid) {
			...
			var endTime =  $("#endTime").val();
			var limit=$("#limit").val();
			$.post("<%=path%>/design/getDurationDateNew.htm",{gridData:gridData,mapData:mapData,createOrModify:"1",dataCreateIdea:"design",
