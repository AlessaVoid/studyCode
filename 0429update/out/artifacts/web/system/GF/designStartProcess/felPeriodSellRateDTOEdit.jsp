...
<script type="text/javascript">
	...
	function showImportDialog(){
		showDialog("<%=path%>/designStartProcess/uploadUI.htm","数据导入",350,130);
	}
	//解析参数模板
	function import_(fileName) {
		Dialog.close();
		$.post("<%=path%>/designStartProcess/readXsl.htm?fileName=" + fileName +"&prodCode="+$("#prodCode").val(),function(result) {
			var a = eval(result);
			showInfo(a);
		}, "json");
	}
...
function sub(formId,url) {
	...
	<body onclick="fmtText()">
		<form id="form1">
			...
			<div align="center">
				<button type="button" onclick="return sub('form1','<%=path%>/designStartProcess/updateFelPeriodSellRateDTO.htm')" class="saveButton"/>
			</div>
		</form>
	</body>
</html>