...
<html>
	...
	<body>
		...
		<div class="box2_custom" boxType="box2" panelTitle="查询条件" id="searchPanel">
			<form action="<%=path%>/webResearchStartProcess/findPage.htm" id="queryForm" method="post">
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/webResearchStartProcess/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onAdd() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/webResearchStartProcess/reSubmitUI.htm?prodCode="+ rows[0].prodCode,"产品预研未启动流程产品重编辑",1280,680);
	}
}
...
function onInfo() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		showDialog("<%=path%>/webResearchStartProcess/infoUI.htm?prodCode=" + rows[0].prodCode,"产品详细信息",1280,680);
	}
}
//删除
function onDelete(){
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
	top.Dialog.confirm('是否删除该产品吗?',	function() {
		$.post("<%=path%>/webResearchStartProcess/deleteProd.htm", {
...
function onExportExcel(){
	top.Dialog.confirm('是否导出全部数据?',	function() {
			$("#queryForm").attr("action","<%=path%>/webResearchStartProcess/exportProdparaFile.htm");
			$("#queryForm").submit();
	});									
}
</script>
