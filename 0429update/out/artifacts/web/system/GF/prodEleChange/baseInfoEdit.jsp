...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript" src="<%=path%>/system/layout/js/prodChange/baseInfoEdit.js"></script>
		<title>基本参数</title> 
...
function initComplete(){
	...
	function sub(formId,url){
		...
		top.Dialog.confirm("确定要保存操作吗?|操作提示", function() {
			...
			if(prodOperModel=="02"||prodOperModel=="04"){
				if($("#profitHandleFlag").val()==""||$("#profitHandleFlag").val()==undefined){
					top.Dialog.alert("非净值型产品必须维护收益计算标识!");
					return false;
				}
			}
			...
			if(valid){
				if(checkData()){
					$("[class*=money]").each(function(){rmoney(this);});
									...
									$.post(url, $("#"+formId).serialize(), function(result) {
										if (result.success == "true" || result.success == true) {
											top.Dialog.alert(result.msg, function() {
												...
												$.post("<%=path%>/gfProdEleChange/selectBaseInfo.htm?prodCode="+$("#prodCode").val(),function(result) {
	...
	function getPeriod(){
		...
		if(prodBeginDate != null && prodBeginDate != "" && prodEndDate != null && prodEndDate != ""){
			...
			if(between_ms<0)top.Dialog.alert("成立日期必须小于到期日期！");
			//计算出相差天数
	...
	<body>
	<form id="form1">
		<input  type="hidden" name="prodCode" value="${prodCode}" id="prodCode"/>
		<input type="hidden" id="uuid" name="uuid" value="${uuid}"/>
		<input type="hidden" id="order" name="order" value="${order}"/>
		<input type="hidden" id="profitHandleFlag_o"value="${entity.profitHandleFlag}"/>
		<input type="hidden" id="prodOperModel_o"value="${entity.prodOperModel}"/>
		<input type="hidden" id="sellFeeType_o"value="${entity.sellFeeType}"/>
		<table class="tableStyle tab-hei-30" width="80%" mode="list" formMode="line">
		<tr>
			<td align="right" width="10%">产品品牌：</td>
			<td width="10%">
				<select prompt="请选择" disabled="disabled" id="brandCode" name="brandCode" selectedValue="${brandCode }" url="<%=path%>/gfProdBrandInfo/getProdBrandInfo.htm" ></select>
			</td>
			<td align="right">
				产品名称后缀：
			</td>
			...
			<td width="16%">
				<dic:select fn="onChange='operModelChange(this.value);'" name="prodOperModel" dicType="PROD_OPER_MODEL" dicNo="${entity.prodOperModel}" tgClass="validate[required]"></dic:select>
		...
		<tr>
			...
			<td>
				<dic:select  name="profitType" dicNo="${entity.profitType}" dicType="PROFIT_TYPE" tgClass="validate[required]" id="profitType"></dic:select>
				<span class="star">*</span>
			</td>
			<td align="right">
				理财产品成立日期：
		...
		<tr>
			<td colspan="8">
				<div align="center">
					<button type="button" onclick="return sub('form1','<%=path%>/gfProdEleChange/compareBaseInfo.htm')" class="saveButton"/>
				</div>
			</td>
		</tr>
	</table>
