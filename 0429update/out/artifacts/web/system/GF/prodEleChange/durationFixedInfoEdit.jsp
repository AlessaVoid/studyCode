		...
		durationGrid = $("#durationParam").quiGrid({
               ...
               { display: '申购日', name: 'purchaseDate', align: 'center', width: "12%",
            	   ...
            	   render : function(rowdata) {
        				return '<div style="margin-right: 35%;" align="right">' + rowdata.purchaseDate.replace(/-/gi, "") + '</div>';}
               },
               { display: '份额确认日', name: 'taCfmDate', align: 'center', width: "12%",
            	   //editor:{type:"date",dateFmt: "yyyyMMdd",detailEdit:"true"},
            	   render : function(rowdata) {
        				return '<div style="margin-right: 35%;" align="right">' + rowdata.taCfmDate.replace(/-/gi, "") + '</div>';}
               },
               ...
               { display: '首次投资周期结束日', name: 'firstInvCycleEndDate', align: 'center', width: "12%",
            	   //editor:{type:"date",dateFmt: "yyyyMMdd",detailEdit:"true"},
            	   render : function(rowdata) {
        				return '<div style="margin-right: 35%;" align="right">' + rowdata.firstInvCycleEndDate.replace(/-/gi, "") + '</div>';}
               },
               { display: '开始时间（从）', name: 'beginTime', align: 'center', width: "15%",
             	   ...
             	   render : function(rowdata) {
             		   if(rowdata.beginTime==null){
            					return '<div style="margin-right: 35%;" align="right">' + "" + '</div>';
             		   }else{
         				return '<div style="margin-right: 35%;" align="right">' + rowdata.beginTime.replace(/-/gi, "") + '</div>';};
             	   	   }
                },
                { display: '结束时间（到）', name: 'endTime', align: 'center', width: "15%",
             	   ...
             	   render : function(rowdata) {
                		if(rowdata.endTime==null){
 						...
 						return '<div style="margin-right: 35%;" align="right">' + rowdata.endTime.replace(/-/gi, "") + '</div>';};
    	   	   			}
                },
...
function sub(formId) {
	...
	$.post("<%=path%>/gfProdEleChange/compareInvCycleInfo.htm?prodCode="+prodCode,param,function(result) {
 			...
 			<input type="hidden" id="openType" name="openType" value="1"/>
   			<table class="tableStyle tab-hei-30" width="80%" mode="list" formMode="line">
				<tr>
					<td>投资周期开始日:</td>
					<td>
					<input type="text" name="firstOpenDay"   id="firstOpenDay" dateFmt="yyyyMMdd" class="date validate[required]" value="${webProdInvCycleRule.firstOpenDay}"/>
					<span class="star">*</span>
					</td>
					<td>投资封闭期:(天)</td>
					<td>
						<input type="text" id="invdays" name="invdays" value="${webProdInvCycleRule.invdays}"  onkeyup="value=value.replace(/[^\d.]/g,'')" maxlength="14" readonly="readonly"/>
						<span class="star">*</span>
					</td>
				</tr>
				<tr>
					<td>开始时间(从):</td>
					<td>
					<input type="text" name="beginTime"   id="beginTime" dateFmt="HHmmss"  class="date" value="${webProdInvCycleRule.beginTime}"/>
					<span class="star">*</span>
					</td>
					<td>结束时间(到):</td>
					<td>
					<input type="text" name="endTime"   id="endTime" dateFmt="HHmmss"  class="date" value="${webProdInvCycleRule.endTime}"/>
					<span class="star">*</span>
					</td> 
				</tr>
				<tr>
				    <td>资金处理方式：</td>
					<td>
					<dic:select dicType="purchase_Amt_Handle_Flag" dicNo="${webProdInvCycleRule.amtHandleFlag}" tgClass="validate[required]" id="amtHandleFlag" name="amtHandleFlag"></dic:select>
					<span class="star">*</span>
					</td>
					<td>份额确认方式：</td>
					<td>
