			...
			<table class="tableStyle tab-hei-30" width="80%" mode="list" formMode="line">
				...
				<tr>
					...
					<td width="25%">
						...
						<dic:select id="prodNameRule" name="prodNameRule" dicType="PROD_NAME_RULE" dicNo="${entity.prodNameRule }"  tgClass="validate[required]" disabled="disabled"></dic:select>
						<span class="star">*</span>
					</td>
					...
					<td width="15%">
						<c:choose>
							<c:when test="${empty entity.accountingFlag}">
								<dic:select name="accountingFlag" id="accountingFlag" dicType="ACCOUNTING_FLAG" onChange="changeDisabled();" dicNo="02" tgClass="validate[required]"></dic:select>
							</c:when>
							<c:otherwise>
				...
				<tr>
					...
					<td width="15%">
						<dic:select id="profitHandleFlag" fn="onChange='deleteProfit(this.value);'" name="profitHandleFlag" dicNo="${entity.profitHandleFlag}" dicType="PROFIT_FLAG" tgClass="validate[required]"></dic:select>
						<span class="star">*</span>
					</td>
					<td align="right" width="10%">销售手续费类型：</td>
					<td width="15%">
						<dic:select id="sellFeeType" fn="onChange='deleteSellFee();'" name="sellFeeType" dicNo="${entity.sellFeeType}" dicType="SELLFEE_TYPE" tgClass="validate[required]"></dic:select>
						<span class="star">*</span>
					</td>
				</tr>
				<tr>
					...
					<td width="15%">
							...
							<c:when test="${empty entity.prodControlFeeRate}">
								<input type="text" id="prodControlFeeRate" name="prodControlFeeRate" value="0.25" class="money validate[length[0,16]]" maxlength="16"/>
							</c:when>
							<c:otherwise>
				...
				<tr>
					<td colspan="4">
						<div align="center" >
							<button type="button" onclick="return sub('form1','<%=path%>/webResearchRollback/updateBaseInfo.htm')" class="saveButton"/>
						</div>
					</td>
				</tr>
			</table>
