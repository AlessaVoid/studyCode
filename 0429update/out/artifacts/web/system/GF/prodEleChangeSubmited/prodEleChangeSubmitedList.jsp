			...
			.quiGrid(
					{
						...
						url : '<%=path%>/eleChangeSubmited/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onInfo() {
	if(selectOneRow(grid)){
		...
		showDialog("<%=path%>/eleChangeSubmited/infoUI.htm?prodCode="+rows[0].prodCode+"&processInstanceId="+rows[0].processInstanceId,
				"详细信息",1280,680);
	}
}
//重编辑
...
function onExportExcel() {
	top.Dialog.confirm('是否导出全部数据?',	function() {
		$("#queryForm").attr("action","<%=path%>/eleChangeSubmited/exportFile.htm");
		$("#queryForm").submit();
	});		
}
