...
<html>
	...
	<body>
		...
		<div class="box2_custom" boxType="box2" panelTitle="查询条件" id="searchPanel">
			<form action="<%=path%>/designAudit/findPage.htm" id="queryForm" method="post">
			...
			.quiGrid(
					{
						...
						url : '<%=path%>/designAudit/findPage.htm',
						sortName : '',
						rownumbers : true,
						checkbox : true,
						height : '100%',
...
function onAudit() {
	...
	if(rows.length == 0){
		...
		showDialog("<%=path%>/designAudit/auditUI.htm?prodCode="+rows[0].prodCode+"&taskId=" + rows[0].taskId, "产品设计审批",1280,680);
	}else{
		var taskId = "";
		...
		top.Dialog.open({
			URL : "<%=path%>/designAudit/mutiAuditUI.htm?menuNo=GF-06-02-01&taskId="+taskId,
			Title : "产品设计多条审批",
			ID:'mutiAppDesign',
			Width : "1280",
			Height : "680"
...
function onInfo() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		showDialog("<%=path%>/designAudit/infoUI.htm?prodCode=" + rows[0].prodCode+"&taskId=" + rows[0].taskId,"产品详细信息",1280,680);
	}
}
...
function onExportExcel() {
	top.Dialog.confirm('是否导出全部数据?',	function() {
		$("#queryForm").attr("action","<%=path%>/designAudit/exportProdparaFile.htm");
		$("#queryForm").submit();
	});	
}
//关闭窗口，用于多条审批成功之后
...
function onViewAppTable() {
	if(selectOneRow(grid)){
		var rows = grid.getSelectedRows();
		$.post("<%=path%>/designAudit/judgment.htm?prodCode=" + rows[0].prodCode,function(result){
			if(result.success=='true'||result.success==true){
				showDialog("<%=path%>/designAudit/prodDesignAppTableInfo.htm?prodCode=" + rows[0].prodCode + "&processInstanceId="+rows[0].processInstanceId,"产品审核表",700,680);
			}else{
...
var rows = grid.getSelectedRows();
	...
	if(rows.length == 0){
		...
		showDialog("<%=path%>/designAudit/designRefTable.htm?prodCodes="+prodCodes+"&currency="+rows[0].currency, "到期参考表",1280,680);
	}else{
