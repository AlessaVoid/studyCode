...
function checkRowHandlerEditor(checked, data) {
	if (checked)
		addCheckedArrayEditor(data[currentEditorId], data[currentEditorName]);
	else
		removeCheckedArrayEditor(data[currentEditorId], data[currentEditorName]);
}
function checkAllRowHandlerEditor(checked) {
	for ( var rowid in this.records) {
		if (checked)
			addCheckedArrayEditor(this.records[rowid][currentEditorId],
					this.records[rowid][currentEditorName]);
		else
			removeCheckedArrayEditor(this.records[rowid][currentEditorId],
					this.records[rowid][currentEditorName]);
	}
}
(function($) {
	...
	var gridHeaderHeight = 46;
	var gridRowHeight = 26;
	if ($("#skin").attr("splitMode") == true
			|| $("#skin").attr("splitMode") == "true") {
	} else {
		...
		if ($parentThemeDom.attr("defaultGridHeaderHeight") != null) {
			gridHeaderHeight = Number($parentThemeDom
					.attr("defaultGridHeaderHeight"));
		}
	...
	$.quiDefaults.Grid = {
		...
		pageSizeOptions : [ 10, 50, 100, 1000 ], //可选择设定的每页结果数
		params : [], //提交到服务器的参数
		columns : [], //数据源
			...
			if ((eval(uncompile("%u02D8%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ"))) == "%u02D5%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3") {
				...
				if (column.editor.link == true) {
					...
					if (column.editor.relation == "child") {
						var parentSelValue = input.parent("td").prev("td")
								.find("select").val()
						...
						input
								.bind(
										"change",
										function() {
											var childSel = input.parent("td")
													.next("td").find("select");
											var fun = column.editor.linkHandler;
											if (column.editor.childDataPath) {
												url = column.editor.childDataPath
														+ input.val();
												jQ
														.ajax( {
															url : url
				...
				if (column.editor.child) {
					if (column.editor.child.type == "filter") {
						...
						if (column.editor.child.itemWidth) {
							child.attr("itemWidth",
									column.editor.child.itemWidth);
						}
						if (column.editor.child.itemHeight) {
							child.attr("itemHeight",
									column.editor.child.itemHeight);
						}
						if (column.editor.child.filterWidth) {
							child.attr("filterWidth",
									column.editor.child.filterWidth);
						}
						...
						if (column.editor.child.fromTitle) {
							child.attr("fromTitle",
									column.editor.child.fromTitle);
						}
						if (column.editor.child.toTitle) {
							child.attr("toTitle", column.editor.child.toTitle);
						}
						if (column.editor.child.listerWidth) {
							child.attr("listerWidth",
									column.editor.child.listerWidth);
						}
						if (column.editor.child.listerHeight) {
							child.attr("listerHeight",
									column.editor.child.listerHeight);
						}
						if (column.editor.child.maxSelection) {
							child.attr("maxSelection",
									column.editor.child.maxSelection);
						}
						child.render();
					} else if (column.editor.child.type == "listerTree") {
						...
						if (column.editor.child.autoParam) {
							child.attr("autoParam",
									column.editor.child.autoParam);
						}
						child.render();
					} else if (column.editor.child.type == "grid") {
						checkedArrayEditor = [];
										...
										.quiGrid( {
											...
											onSelectRow : function() {
												if (!column.editor.multiMode) {
													var myEditorGridRow = myEditorGrid
															.getSelectedRow();
													input
															.find(
																	"input[type=text]")
															.val(
																	myEditorGridRow[currentEditorName]);
													input
															.attr(
																	"relValue",
																	myEditorGridRow[currentEditorId]);
													input
															.attr(
																	"relText",
																	myEditorGridRow[currentEditorName]);
					...
					if (column.editor.multiMode == true) {
						input
								.bind(
										...
										function() {
											if (column.editor.child.type == "filter"
													|| column.editor.child.type == "lister"
													|| column.editor.child.type == "listerTree") {
												input
														.find(
																"input[type=text]")
														.val(
																child
																		.attr("relText"));
												input.attr("relValue", child
														.attr("relValue"));
												input.attr("relText", child
														.attr("relText"));
											} else if (column.editor.child.type == "grid") {
												if (column.editor.multiMode) {
													input
		...
		create : function(container, editParm, g, p) {
			if ((eval(uncompile("%u02D8%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ"))) == "%u02D5%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3") {
				...
				if (column.editor.multiListHeight) {
					input
							.attr("multiListHeight",
									column.editor.multiListHeight);
				}
					...
					$.quiui.core.UIComponent,
					{
						...
						_init : function() {
								...
								p.detail = {
									...
									onShowDetail : function(record, container,
										...
										$("<div class='l-clear'></div>")
												.appendTo(container);
										var $saveBtn = $("<button type='button'><span  class='icon_save'>"
												+ p.saveMessage
												+ "</span></button>");
										$saveBtn.render();
										$saveBtn
										...
										//添加提交按钮
										var $okBtn = $("<button type='button'><span  class='icon_ok'>"
												+ p.applyMessage
												+ "</span></button>");
										$okBtn.render();
										$okBtn.appendTo(container).click(
												function() {
													g.submitEdit(record);
												});

										//添加取消按钮
										var $cancelBtn = $("<button type='button'><span  class='icon_no'>"
												+ p.cancelMessage
												+ "</span></button>");
										$cancelBtn.render();
										$cancelBtn.appendTo(container).click(
												function() {
													g.cancelEdit(record);
													removeRow();
												});
							...
							if (p.tree)//树表格
							{
								...
								p.tree.iconClass = p.tree.iconClass
										|| function(rowData) {
											var iconClassName = "";
						...
						_render : function() {
							...
							if (p.multihead) {
								...
								gridhtmlarr
										.push("                      <div class='l-grid-header l-grid-header-multi'>");
							} else {
								gridhtmlarr
							...
							if (p.showPageInfo) {
								...
								gridhtmlarr
										.push("                      <div class='l-bar-group  l-bar-message' style='display:none;'><span class='l-bar-text'></span></div>");
							}
							if (p.showPageSize) {
								...
								gridhtmlarr
										.push("            <div class='l-bar-group l-bar-selectpagesize' style='display:none;'></div>");
								gridhtmlarr
										.push("                <div class='l-bar-separator' style='display:none;'></div>");
							}
						...
						_createEditor : function(editor, container, editParm,
								width, height) {
							var g = this, p = this.options;
							var editorInput = editor.create(container,
									editParm, g, p, editParm.value);
							if (editor.setValue)
						...
						endEdit : function(rowParm) {
							...
							if (g.editor.editing) {
								var o = g.editor;
								//触发事件
								if (g.trigger('sysEndEdit',
										[ g.editor.editParm ]) == false) {
									return false;
								}
								//触发事件
						...
						setScroller : function(scrollY, scrollX, animate) {
							...
							if (scrollY && typeof (scrollY) == "string") {
								if (scrollY == "bottom") {
									scrollYNum = g.gridbody.find(
											".l-grid-body-inner").height();
								} else if (scrollY == "top") {
									scrollYNum = 0;
								}
							...
							if (scrollX && typeof (scrollX) == "string") {
								if (scrollX == "right") {
									scrollXNum = g.gridbody.find(
											".l-grid-body-inner").width();
						...
						collapseDetail : function(rowParm) {
							...
							for ( var i = 0, l = g.columns.length; i < l; i++) {
								if (g.columns[i].isdetail) {
									...
									indexInDetailExtendRow = $.inArray(rowdata,
											g.detailExtendRow);
									if (indexInDetailExtendRow != -1) {
										g.detailExtendRow.splice(
												indexInDetailExtendRow, 1);
									}
							...
							for ( var i = 0, l = children.length; i < l; i++) {
								...
								//折叠
									...
									var currentRow2 = $( [ g
											.getRowObj(o['__id']) ]);
									if (g.enabledFrozen())
										currentRow2 = currentRow2.add(g
												.getRowObj(o['__id'], true));
									$(".l-grid-tree-link", currentRow2)
											.removeClass(
													"l-grid-tree-link-open")
							...
							if (typeof (colwidth) == "string"
								...
								column._width = colwidth = parseInt(parseInt(colwidth)
										...
										* (g.grid.width() - g.columns.length
												- rownumbersColFix - checkboxColFix));
						...
						_createHeaderCell : function(column) {
							...
							if (column.ischeckbox) {
								...
								jcell
										.html("<div class='l-grid-hd-cell-inner'><div class='l-grid-hd-cell-con-checkbox'><span class='l-grid-hd-cell-text l-grid-hd-cell-btn-checkbox'></span></div></div>");
							}
							if (column.isdetail) {
							...
							$(".l-grid-hd-cell-text:first", jcell).html(
									headerText);
							if (column.isSort) {
								$(".l-grid-hd-cell-text:first", jcell).css(
										"cursor", "pointer");
							}
						...
						_initFootbar : function() {
							...
							if (p.usePager) {
								...
								$('.l-bar-selectpagesize', g.toolbar).append(
										"<select name='rp' keepdefaultstyle='true'>" + optStr
												+ "</select>");
								if (selectedIndex != -1)
										...
										.each(
												function(i, item) {
													...
													gridhtmlarr
															.push(' groupindex"=' + i + '" >');
													gridhtmlarr
															.push('<td colSpan="' + g.columns.length + '" class="l-grid-grouprow-cell">');
													gridhtmlarr
						...
						_getHtmlFromData : function(data, frozen) {
							...
							for ( var rowparm in data) {
								...
								gridhtmlarr.push('<tr ');
								//}
								gridhtmlarr.push(' id="' + g._getRowDomId(item,
										frozen) + '"');
								var rowPosition = p.rowHeight
										* (item['__index'] + 1)
										+ item['__index'] - p.rowHeight;
								...
								$(g.columns)
										.each(
												function(columnindex, column) {
													...
													if (this.isrownumber) {
														gridhtmlarr
																.push(' class="l-grid-row-cell l-grid-row-cell-rownumbers" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner"');
														if (p.fixedCellHeight)
															gridhtmlarr
																	.push(' style = "height:' + p.rowHeight + 'px;" ');
														gridhtmlarr
																.push('>' + (parseInt(item['__index']) + 1) + '</div></td>');
														return;
													}
													//如果是复选框(系统列)
													if (this.ischeckbox) {
														gridhtmlarr
																.push(' class="l-grid-row-cell l-grid-row-cell-checkbox" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner"');
														if (p.fixedCellHeight)
															gridhtmlarr
																	.push(' style = "height:' + p.rowHeight + 'px;" ');
														gridhtmlarr
																.push('><div class="l-grid-row-cell-con-checkbox" ><span class="l-grid-row-cell-btn-checkbox"></span></div></div></td>');
														return;
													}
													//如果是明细列(系统列)
													else if (this.isdetail) {
														gridhtmlarr
																.push(' class="l-grid-row-cell l-grid-row-cell-detail" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner"');
														if (p.fixedCellHeight)
															gridhtmlarr
																	.push(' style = "height:' + p.rowHeight + 'px;" ');
														gridhtmlarr
								...
								if (item["appendRowHtml"]) {
									...
									gridhtmlarr
											.push('"><td colspan="6" class="l-grid-row-cell-appendRow">');
									if (rightContent == 1) {
						...
						_getCellHtml : function(rowdata, column) {
							var g = this, p = this.options;
							if (column.isrownumber)
								return '<div class="l-grid-row-cell-inner" style="height:'
										+ p.rowHeight
										+ 'px;">'
							...
							//htmlarr.push('<div');
							htmlarr
									.push(' style = "width:' + parseInt(column._width - 8) + 'px;');
							if (p.fixedCellHeight)
						...
						_applyEditor : function(obj) {
							...
							if (prevRow.hasClass("l-grid-row")) {
								g.nearCells
										.push(prevRow.find("td").eq(cellIdx)[0]);
							} else {
								g.nearCells.push(null);
							}
							if (nextRow.hasClass("l-grid-row")) {
								g.nearCells
										.push(nextRow.find("td").eq(cellIdx)[0]);
							} else {
								g.nearCells.push(null);
							}
						...
						_getTotalSummaryHtml : function(data, classCssName,
							...
							if (classCssName)
								totalsummaryArr
										.push('<tr class="l-grid-totalsummary ' + classCssName + '">');
							else
								totalsummaryArr
										.push('<tr class="l-grid-totalsummary">');
							$(g.columns)
									.each(function(columnindex, column) {
										if (this.frozen != frozen)
											return;
										//如果是行序号(系统列)
											if (this.isrownumber) {
												totalsummaryArr
														.push('<td class="l-grid-totalsummary-cell l-grid-totalsummary-cell-rownumbers" style="width:' + this.width + 'px"><div>&nbsp;</div></td>');
												return;
											}
											//如果是复选框(系统列)
											if (this.ischeckbox) {
												totalsummaryArr
														.push('<td class="l-grid-totalsummary-cell l-grid-totalsummary-cell-checkbox" style="width:' + this.width + 'px"><div>&nbsp;</div></td>');
												return;
											}
											//如果是明细列(系统列)
											else if (this.isdetail) {
												totalsummaryArr
														.push('<td class="l-grid-totalsummary-cell l-grid-totalsummary-cell-detail" style="width:' + this.width + 'px"><div>&nbsp;</div></td>');
												return;
											}
											totalsummaryArr
											...
											if (columnname) {
												totalsummaryArr
														.push('columnname="' + columnname + '" ');
											}
											totalsummaryArr
													.push('columnindex="' + columnindex + '" ');
											totalsummaryArr
													.push('><div class="l-grid-totalsummary-cell-inner"');
											if (column.align)
												totalsummaryArr
														.push(' style="text-Align:' + column.align + ';"');
											totalsummaryArr.push('>');
								...
								if (g.editor.editing && !$.quiui.win.masking) {
									if (keyCode == 37) {//左
										...
										if (g.nearCells[2] != null) {
											var minRowIdx = Math
													.round(g.gridbody[0].scrollTop
															/ (p.rowHeight + 1)) + 1;
											if (g.nearCells[4] == minRowIdx) {
												g.setScroller(
														g.gridbody[0].scrollTop
																- p.rowHeight
																- 1, null,
														false);
												setTimeout(
						...
						_onClick : function(e) {
							...
							if (src.row && p.detail && p.detailClickExpand) {
								...
								var indexInDetailExtendRow;
								//得到该行数据在g.collapsedRows中的索引
								indexInDetailExtendRow = $.inArray(item,
										g.detailExtendRow);
								//没打开
								if (indexInDetailExtendRow == -1) {
									//关闭其他
									if (p.detailCloseOther) {
										for ( var j = 0; j < g.detailExtendRow.length; j++) {
											g
													.collapseDetail(g.detailExtendRow[j]);
										}
										g.detailExtendRow = [];
									}
									g.extendDetail(item);
						...
						addItem : function(item) {
							...
							if (item.line) {
								if (p.position == "right"
										|| item.position == "right") {
									g.toolBar
											.append('<div class="l-bar-separator-right"></div>');
								} else {
									g.toolBar
