 ...
 */
@Service
public class WebLoanPlanAuditServiceImplBak {
    @Autowired
    ...
    public PlainResult<String> startLoanPlanAuditProcess(String organCode, String planOper, String planOperName, String assignee, String planId) {
        ...
        try {
            ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(AuditMix.PLAN_TOTAL_MECH, new HashMap<>());
            ...
            tbAuditRecordHistDO.setTaskId(task.getId());
            ...
            tbAuditRecordHistDO.setAssigneeName(planOperName);
...
//         if (LoanStateEnums.STATE_APPROVING.status == planDetailDO.getPdState()) {
//             planTaskInfoDTO.setPlanStatus(LoanStateEnums.STATE_APPROVING.description);
//         } else if (LoanStateEnums.STATE_APPROVED.status == planDetailDO.getPdState()) {
//             planTaskInfoDTO.setPlanStatus(LoanStateEnums.STATE_APPROVED.description);
//         } else if (LoanStateEnums.STATE_DISMISSED.status == planDetailDO.getPdState()) {
//             planTaskInfoDTO.setPlanStatus(LoanStateEnums.STATE_DISMISSED.description);
//         }
//
//         planTaskInfoDTO.setPlanCreateTime(tbPlanDO.getPlanDateStart());
//         planTaskInfoDTO.setPlanEndTime(tbPlanDO.getPlanDateEnd());
// //        double reqTotalExpire = tbReqDetailMapper.sumReqExpire(tbReqDetailQuery);
// //        double reqTotalAmount = tbReqDetailMapper.sumReqAmount(tbReqDetailQuery);
// //        planTaskInfoDTO.setReqTotalAmount(String.valueOf(reqTotalAmount));
// //        planTaskInfoDTO.setReqTotalExpire(String.valueOf(reqTotalExpire));
//         return planTaskInfoDTO;
        return null;
    }
     ...
     * @return ListResult
     */
    public ListResult<PlanTaskInfoDTO> getAllRollbackTaskList(String assignee, int pageNo, int pageSize) {
    ...
    public void completeTaskAudit(String taskId, String comment, Map<String, Object> varMap, boolean lastUserType) {
        ...
        TbAuditRecordHistDO tbAuditRecordHistDO = new TbAuditRecordHistDO().setBusinessId(planId).setProcessKey(AuditMix.PLAN_TOTAL_MECH).setProcessInstanceId(processInstanceId).setTaskId(task.getId());
        FdOper fdOper = getOperInfoByOperCode(lastUser);
        Preconditions.checkArgument(Objects.nonNull(fdOper), "no match oper info find");
        tbAuditRecordHistDO.setAssigneeName(fdOper.getOpername());
        tbAuditRecordHistDO.setStartAt(LocalDateTime.now());
        switch (isAgree) {
            case AuditMix.AUDIT_REJECT:
                tbAuditRecordHistDO.setAssignee(lastUser + AuditMix.COLON + AuditMix.AUDIT_REJECT_SUFFIX);
                tbAuditRecordHistDO.setStatus(LoanStateEnums.STATE_DISMISSED.status);
                tbAuditRecordHistMapper.insertRecordHist(tbAuditRecordHistDO);
                tbPlanDO.setPlanStatus(LoanStateEnums.STATE_DISMISSED.status);
                tbAuditRecordHistMapper.updateRecordHistAuditState(tbAuditRecordHistDO);
                tbAuditRecordHistMapper.updateRecordHistPosition(tbAuditRecordHistDO);
                break;
            case AuditMix.AUDIT_PASS:
                tbAuditRecordHistDO.setAssignee(lastUser + AuditMix.COLON + AuditMix.AUDIT_PASS_SUFFIX);
                if (lastUserType) {
                    tbAuditRecordHistDO.setStatus(LoanStateEnums.STATE_APPROVED.status);
                    tbAuditRecordHistDO.setEndAt(LocalDateTime.now());
                    tbAuditRecordHistMapper.insertRecordHist(tbAuditRecordHistDO);
                    tbAuditRecordHistMapper.updateRecordHistAuditState(tbAuditRecordHistDO);
