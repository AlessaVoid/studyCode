 ...
 **/
@Service
public class CreditPlanServiceImpl extends GenericService<TbPlan, String> implements CreditPlanService {

    public static Logger logger = Logger.getLogger(CreditPlanServiceImpl.class);
    ...
    public PlainResult<String> addCreditPlan(HttpServletRequest request, String operCode, String organCode, Integer planType,  String organlevel, String upOrgan) {
        ...
        try {
            ...
            String combLevel = request.getParameter("combLevel");

            //校验特殊字段非空
            if (StringUtils.isEmpty(planMonth) || StringUtils.isEmpty(planUnit) || StringUtils.isEmpty(planDetailStr) || StringUtils.isEmpty(amountCount)||StringUtils.isEmpty(combLevel)) {
            ...
            if (CollectionUtils.isNotEmpty(tbPlanListCheck)) {
                return result.error(HttpServletResponse.SC_FORBIDDEN, "所属月份机构计划已存在");
            }
            ...
            if ("1".equals(organlevel)) {
                ...
                if (CollectionUtils.isNotEmpty(tbPlanListCheck2)) {
                    return result.error(HttpServletResponse.SC_FORBIDDEN, "所属月份已经制定条线计划，不能再制定机构计划");
                }
            }
    ...
    public PlainResult<String> updateCreditPlan(HttpServletRequest request, String operCode, String organCode, String organlevel, String upOrgan) {
        ...
        try {
            ...
            if ("1".equals(organlevel)) {
                ...
                int combLevelOne = plan.getCombLevel();
                //总行贷种级别
                TbPlan planParam = new TbPlan();
                planParam.setPlanMonth(plan.getPlanMonth());
                planParam.setPlanOrgan(upOrgan);
                planParam.setPlanType(TbPlan.PLAN);
                TbPlan upPlan = tbPlanMapper.selectByAttr(planParam).get(0);
                int combLevelTotal = upPlan.getCombLevel();
     ...
     * @return
     */
    private List<TbPlanDetail> buildTbPlanDetail(String planDetailStr, TbPlan tbPlan) {
