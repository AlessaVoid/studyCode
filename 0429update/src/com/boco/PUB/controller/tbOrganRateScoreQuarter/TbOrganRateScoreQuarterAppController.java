...
/**
 * @Author: liujinbo
 * @Description: 提交审批季度评分
 * @Date: 2020/02/05
 * @Version: 1.0
 */
@Controller
@RequestMapping("/tbOrganRateScoreQuarterApp")
public class TbOrganRateScoreQuarterAppController extends BaseController {
    ...
    @SystemLog(tradeName = "提交季度评分页面", funCode = "AL-03-01-03", funName = "提交季度评分页面", accessType = Dic.AccessType.READ, level = Dic.Debug.DEBUG)
    public String commitTbPlanUI(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        return basePath + "/AL/tbOrganRateScoreQuarter/tbOrganRateScoreQuarterSubmit/tbOrganRateScoreQuarterDetailCommitPage";

    }

    @ResponseBody
    @RequestMapping("/startTbOrganRateScoreMonthAudit")
    @SystemLog(tradeName = "启动审批流程", funCode = "AL-04-01", funName = "启动审批流程", accessType = Dic.AccessType.WRITE, level = Dic.Debug.DEBUG)
    public String startLoanReqAudit(String id, String auditOper,String comment) throws Exception {

        PlainResult<String> result = tbOrganRateScoreService.startRateScoreAuditProcessQuarter(id, getSessionOperInfo().getOrganCode(),
