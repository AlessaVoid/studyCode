 ...
 * </pre>
 */
public class DicOutsTag extends BodyTagSupport {
	...
	public int doEndTag() throws JspException {
		try {
			String dicNo = this.DicNo.toString();
			String[] dicNos = dicNo.split(",");
			...
			if(list != null){
				for (int i = 0; i < list.size(); ++i) {
					...
					if (ontTag.get("DICT_KEY_IN") != null){
						for(int n=0;n<dicNos.length;n++){
							if(ontTag.get("DICT_KEY_IN").toString().trim().equals(dicNos[n].trim())){
								tag =tag + "，" + (String) ontTag.get("DICT_VALUE_IN");
							}
						}
					}
				}
